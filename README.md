...
> Two knobs cut across all four levels (they aren't levels themselves): **slots** decide *what* runs at each stage, and **appetite** sets *how much rope* the pipeline gets before checking back. See [Configuration](docs/configuration.md).
...
1. **Tell it where your stuff lives.** Run `/faff-onboard` to write a `.faffrc.yaml` at your repo root (or drop in three lines by hand — see [Configuration](docs/configuration.md)).
...
Each step offers to chain into the next, so you can just keep saying yes. That's the whole loop. For worked examples, see the [Walkthroughs](docs/walkthroughs.md).
...
- [The `faff` CLI](docs/cli.md) — the bundled command-line tool and its subcommands, grouped by purpose.
- [Architecture](docs/architecture.md) — the segregated orchestrator / implementor / evaluator agent lanes.
