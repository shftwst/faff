...
## Addendum — local-LLM kernel tool-routing measured: GO for a cheap CI smoke lane (FAFF-129, 2026-06-16)

This closes **costed follow-up #4** (the third open lane above): can a non-frontier **local model** tool-route the tidy kernel faithfully enough to be a near-free plumbing-regression harness? **Result: yes — `qwen3.6:27b-mlx` drove the kernel at 100% tool-call fidelity and 0% decision flakiness over 5 reps, matching this ADR's frontier 0% baseline.** It is a **GO** for the narrow purpose below.

**Purpose (sharpened with the operator).** The *authoritative* post-refactor regression reverify — the one guarding the upcoming aggressive lean rewrite of the whole skills-prose suite — stays a **frontier apples-to-apples** run (it must match the frontier baseline). This lane is **only the cheap canary**: does a world exist where an *occasional, free, local* smoke check could catch gross plumbing breakage cheaply in CI? That is the question this probe answers, and the bar it is judged against (smoke-reliable, not authoritative).

**Mechanism finding (load-bearing — the spec's Chosen path changed).** The spec first chose agentic `claude -p` over ollama's env-redirect. In the build this proved **impractical for a reasoning model**: `claude -p` can only reach ollama via the Anthropic **`/v1/messages`** endpoint, which carries **no thinking toggle**, and `qwen3.6:27b-mlx` then thinks on every turn — measured ~30 s/turn (vs ~2–6 s with thinking off), and a *trivial* connectivity prompt timed out at 240 s. ollama can disable thinking only per-request on the native `/api/chat` (`think:false`) or per interactive session (`/set nothink`) — neither reachable through `claude -p`. **Resolution:** drive the kernel with a **native `/api/chat` agentic tool-loop exposing a single generic `bash` tool**, so the model issues the *same* `./bin/tracker` / `./bin/faff` Bash calls a `claude -p` agent would (identical seam capture and fidelity metric) — over a transport where `think:false` works. This is the same agent *behaviour*, a faster *transport*. (`ollama launch claude` is just `claude` with the same env-redirect, so it inherits the same `/v1/messages` limitation — confirmed, not a workaround.)

**Substrate & oracle.** The ADR-0003 7-issue throwaway substrate, reconstructed (mock `tracker.json`; the `./bin/tracker` + `./bin/faff` logging ports; the published oracle `ready=[ISS-1]`, `on_hold=[ISS-2]`, `park_clear=[ISS-3]`, `blocked=[ISS-4]`, `needs_prep=[ISS-6]`; mutations `setStatus ISS-1→Todo`, `removeLabel faff-parked ISS-3`). The substrate was **verified to reproduce that oracle deterministically** (running the kernel via the real `faff next`/`eligible`) before any model rep. Scratch, uncommitted; raw rep artifacts gitignored under `eval/report/faff-129-local-kernel/`.

**Measurement (5 reps, `qwen3.6:27b-mlx`, `think:false`, native `/api/chat` loop).**

| rep | turns | faff seam calls | well-formed | bypass | tool-call fidelity | decision vs oracle | wall |
|---|---|---|---|---|---|---|---|
| smoke | 11 | 8 | 8 | 0 | 1.00 | ✅ exact | 168 s |
| 1 | 12 | 9 | 9 | 0 | 1.00 | ✅ exact | 214 s |
| 2 | 5 | 9 | 9 | 0 | 1.00 | ✅ exact | 169 s |
| 3 | 6 | 8 | 8 | 0 | 1.00 | ✅ exact | 182 s |
| 4 | 11 | 8 | 8 | 0 | 1.00 | ✅ exact | 160 s |

**AGG tool-call fidelity 1.00 · decision fidelity 1.00 (n=5, 0 bypass, ~3 min/rep).** Every `faff` call was well-formed (correct subcommand + flags — `--not-eligible` on the unblessed ISS-2, `--parked` on ISS-3, `--blocked` on ISS-4) and routed through the port; no rep read `tracker.json` directly or hand-guessed a verdict. **Path** varied (8 vs 9 calls, 5–12 turns — the benign redundant-`eligible` variance this ADR's run 2 also showed) but **decisions did not** — every rep's buckets+mutations were byte-identical to the oracle, including the two edge cases (the parked-but-stale ISS-3 → `removeLabel`, and the genuinely-blocked ISS-4). This **measures tool-call fidelity first** (the issue's primary guardrail): fidelity is 1.00, so the decision-fidelity number is trustworthy and not masking false flakiness.

**`think:false` adequacy (the obvious objection).** Disabling thinking is Qwen3's *trained non-thinking mode* — genuinely less deliberation, not hidden tokens — so it can hurt on reasoning-heavy tasks. It is adequate **here** because the kernel deliberately offloads every judgement to the deterministic CLI; the model only has to route. Confirmed empirically on the host: `think:false` answered the kernel's own sub-determinations correctly ("is A blocked?" with one open + one Done blocker → `yes`; "is the park stale?" with the only blocker Done → `remove`) in 1.6–5.6 s, whereas `think:true` took ~30 s and burned its whole budget thinking. For a *judgement* surface this would not hold — which is exactly why judgement-evals stay on the frontier (ADR 0004 / FAFF-130).

**Decision — GO (for the cheap-canary purpose only).** A free local model is a faithful tool-router for the deterministic kernel: viable as an **occasional, on-demand / CI smoke check** that catches gross seam-plumbing breakage during the lean prose refactor, with **no subscription/rate-limit-pool draw**. It is **not** a replacement for the frontier reverify (the authoritative gate) and **not** a judgement check.

**Caveats / what this did NOT establish.**
1. **Scoped brief, not the real prose.** Reps drove a *scoped kernel brief* (as ADR 0003 did), not the real 47 KB `faff-tidy/SKILL.md`. So this proves the model's tool-routing *competence*; to actually catch *lean-refactor* regressions the productionised lane must drive the **real skill prose** (the thing being refactored) — the load-bearing next step.
2. **Transport differs from the frontier live-driver lane.** The shipped live-driver (`eval/cli-driver.mjs`) uses `claude -p`; this local lane uses a native `/api/chat` tool-loop. A productionised local smoke lane is a *new* driver (closest to `makeDirectOllamaDriver`, FAFF-136, but agentic), not a `--driver local` flip of the existing runner.
3. **N=5, one model, one substrate** — a directional go/no-go, not a distribution; CI-gating stays off until proven (unchanged).
4. **Config-isolation is moot on this path** (a side benefit): the native loop never spawns `claude` and never touches `~/.claude.json` or any Anthropic credential (it talks to ollama directly, no auth token needed), so the FAFF-138 per-rep `CLAUDE_CONFIG_DIR` race and credential-forwarding hazards simply do not arise.

**Costed follow-up (recommended).** Productionise the local smoke lane: an agentic `/api/chat` tool-loop driver (`think:false`) that drives the **real** tidy kernel prose over a small committed fixture, asserts decision-equality against the `faff next`/`eligible` oracle, and runs on-demand (and, once trusted, as a non-gating CI smoke job). Wire it as an additive lane alongside the existing `eval/run-live-evals.mjs` `LIVE_KINDS`. Feeds the FAFF-93 live-driver decision: the *plumbing* lane can be free; the *judgement* lane stays frontier. Every number above traces to a real rep under `eval/report/faff-129-local-kernel/`; nothing was fabricated.
