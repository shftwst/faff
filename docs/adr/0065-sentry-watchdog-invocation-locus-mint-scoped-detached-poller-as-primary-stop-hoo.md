...
Every candidate against all seven decision criteria (1 Independence · 2 Detection→action gap · 3 Liveness-contract fit · 4 Lifecycle · 5 Install wiring · 6 Un-subvertability + isolation posture · 7 Proportionality). ✅ pass · ⚠️ partial · ❌ fail (defining-criterion failure on 1 or 3 is disqualifying on its own).

| Candidate | 1 Independence | 2 Action gap | 3 Liveness fit | 4 Lifecycle | 5 Install wiring | 6 Isolation posture | 7 Proportionality |
|---|---|---|---|---|---|---|---|
| **Hook (Stop-hook staleness, standalone)** | ⚠️ only fires if *another* session reaches turn-end; the stuck session's own Stop never fires while stuck; zero-other-session case (the solo overnight run) is uncovered | ✅ can call `sentry abort` | ✅ reads the same ledger | ✅ stateless, no lifecycle to own | ✅ reuses hooks-ensure.js verbatim | ⚠️ in-container, shares build lane's uid/fs (FAFF-324 residual) | ✅ near-zero marginal cost |
| **Daemon (mint-scoped detached poller)** | ✅ probe-demonstrated: survives launching-process exit, polls with zero further cooperation | ✅ calls unmodified `sentry abort` | ✅ reads the same ledger | ✅ probe-demonstrated clean sentinel stop; start/stop owned by run-mint/run-done | ⚠️ new spawn+lifecycle wiring in beep-boop skill prose, but no new install ceremony (no settings.json entry, no host installer) | ⚠️ in-container, shares build lane's uid/fs (same FAFF-324 residual as hook) | ✅ smallest mechanism that actually satisfies criterion 1 for the solo-run case |
| **Cron / OS scheduler** | ✅ survives everything, including container restart if host-level | ✅ could call `sentry abort` | ✅ reads the same ledger (if the bind-mounted path is wired in) | ⚠️ runs independent of any single faff run's lifecycle; needs its own per-run scoping logic to avoid firing after run-done | ❌ no cross-platform idempotent install precedent; ADR-0010 declines host-level infra ownership | ✅ the only candidate that can genuinely cross the container boundary (host-level install) | ❌ install cost far exceeds the daemon's for the same criterion-1 property |
| **CI-side (`schedule:`-triggered Actions)** | ✅ would run with zero cooperation, in principle | ❌ no in-flight action path (CI can't reach the live worktree to abort) | ❌ `.faff/` is confirmed gitignored — CI has no path to `owner.last_heartbeat` at all | ❌ no run-scoped lifecycle possible without a ledger it can see | ❌ no `schedule:` trigger exists in either workflow today | N/A (fails 1&3 before isolation posture is reachable) | ❌ disqualified structurally, not a cost tradeoff |
| **Alert-only composite (fallback if every locus above failed)** | — | ⚠️ alert only, no abort | ✅ | ✅ | ✅ | ⚠️ | — |

**Reading the table:** CI fails two defining criteria (1 is nominally passable but moot once 3 fails outright) and is rejected structurally. Cron passes 1 cleanly and is the only candidate to pass 6 for real, but fails 5 hard enough (no precedent, contradicts ADR-0010's chosen boundary model) that it loses on proportionality (7) to the daemon, which delivers the same criterion-1 property this spike exists to secure at a fraction of the wiring cost. Hook alone fails criterion 1 for the exact run shape (solo overnight `/faff-beep-boop`) the spec's WHY names as the motivating case, so it is retained only as a cheap assist, not the answer. No candidate scores clean on 6 except host-level cron (rejected on 5/7) — the daemon+hook composite's 6-score is ⚠️, not ✅, and this ADR says so plainly rather than overclaiming un-subvertability the container model doesn't actually deliver. Because the daemon delivers a real, actionable, non-null answer to the defining criterion (1), the alert-only composite is not needed as the settled decision — it is recorded in the table only because the spec requires it be scored as a fallback candidate.
