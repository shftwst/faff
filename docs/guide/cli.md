...
| Subcommand | What it does |
|---|---|
| `contract <name> [--in FILE]` | Per-slot contract script: extraction JSON in → canonical contract data out (`spec-readiness`, `review-verdict`, `quality-gates`, `delivery-outcome`, `prd-readiness`, `prdr-admission`, `prdr-yagni`, `prd-coverage`); exit 0 conformant / 1 non-conformant / 2 fail-loud. |
