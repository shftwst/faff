...
The reference workflows under `operations/ci/` (`l3-watcher.yml` and
...
- **The background-task wait ceiling** (`CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0` on the `claude -p "/faff-beep-boop"` line). The harness terminates the running process when a background task outlives this ceiling, which **defaults to 600s** when unset. A legitimately slow run-end step — post-merge verification can take several minutes against GitHub CI — can outlive 600s and force-terminate the *parent orchestrator itself* mid-wait, leaving the run-ledger stuck at `owner.status: "running"` with no recorded outcome even though the PR already merged. Setting it to `0` removes that inner ceiling, so the single outer `timeout 300m` stays the *only* wall-clock bound rather than a second, shorter, silent one truncating legitimate slow steps. If you would rather keep a finite ceiling, set one comfortably above post-merge-check's worst case — but then keep it above the outer `timeout` in effect, or you reintroduce the same truncation. Either way, `disposition` now distinguishes a merged-but-unclosed run (a `merged-unclosed` attention item) from a genuine build failure, so a truncation after a real merge reads as a less-alarming close-the-ledger task rather than a red build.
- **Auto-recovering a merged-but-unclosed run.** The wrapper runs `faff reconcile-recover` in the disposition arm, between the drain and the final `faff disposition` exit: for each `merged-unclosed` item `faff disposition --json` surfaces (the previous bullet's case — a durable merge with no recorded ledger close), it closes the run's ledger to `shipped` on your behalf, but **only** behind a freshly re-run `post-merge-check` returning `verified-ok`. A red or absent re-verification **blocks** the close and leaves the `merged-unclosed` attention item standing exactly as before — the auto-close never masks a regression, it only removes the manual step for the safe, provable case. `l3-watcher.yml` wires the same call as its own step between the drain and disposition. Both reference wrappers are in-repo only: a self-hosted drain wrapper deployed outside this repo needs its own operator applying the same `faff reconcile-recover` call to its disposition-non-zero arm by hand. The verb's scoped write-authority (which actor, which fields, under what precondition) is recorded in the committed architecture-decision log alongside this repo's other named exceptions to the run-ledger's write-authority rule.
...
- **For the L4 shape** (resume-segmentation across firings), the wrapper stays L3; the resolve-open-run-or-mint and the resume-vs-terminal reconciliation live in `operations/ci/l4-watcher.yml` and translate to the same shell. That path additionally needs a **persistent, non-cleaned single-runner workspace** (the ledger under `.faff/runs/` must survive between firings for `lights-out --resume`) and a **sequential** dispatch slot.
