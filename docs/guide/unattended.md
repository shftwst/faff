...
**This `$FAFF_RUN_DIR` is the ledger that actually drains — the whole run, one ledger, reused end-to-end.** `faff-beep-boop` §0a inherits the exact directory you captured above and reuses it; it never mints a second one, so the run dir you're holding is always the one `disposition` reads. Because of that, `disposition` resolves **only** `--run-dir` → `$FAFF_RUN_DIR` — with neither supplied it exits 3 rather than guessing at the newest `*-lights-out` dir under `.faff/runs`, since several unclosed ledgers can legitimately coexist (a run that refused before reaching the tracker, a still-in-flight sibling) and "newest" is not a safe stand-in for "the one you launched."
...
**Giving an unattended L3 run the Sentry kill-switch.** By default L3's Sentry is **advisory** — it logs a derailment for the morning review but does not *kill* the run live, because L3 assumes a human on the loop. That assumption breaks for a watcher nobody is watching overnight. The Sentry abort kill-switch is keyed on **attendedness**, not the L4 mint: set `autonomous.unattended: true` in the **base `.faffrc.yaml`** to declare the run **unattended**, and its Sentry **aborts** on a trip exactly as L4 does — the kill-switch on the real axis rather than the full L4 mandate. Put it in the **base** file, not the `.faffrc.local.yaml` overlay: the abort-acting loci read governance config (`budget.*`/`sentry.*`) from the base only, so an overlay-only declaration would show in the `faff config resolved` banner yet never actually arm the kill-switch. It is fail-safe off: only a literal `true` arms it, and a typo or unset value leaves the **attended → advisory** default in force (a base-file value is echoed in the `faff config resolved` run banner so you can confirm it took). An **attended** L3 run (the default — you declare nothing) stays advisory, because the human at the keyboard is the kill-switch. The declaration covers both safe stops: **`abort`** (whole-run kill-switch) and **`pause`** (parks the one implicated issue — a `fix-review-thrash`/respec-treadmill build, `scope-drift`, or a member-scoped stall — and keeps draining the rest of the queue). The finer `surface`/`correct` interventions stay L4-only, so an unattended L3 run gains both live safety stops without the adversarial steering. This is what the **self-directed faff-on-faff watcher** most wants: that watcher *cannot* be L4 (L4 refuses a self-directed run), so before this it could never get a kill-switch however unattended it ran. *(The earlier `autonomous.sentry_acting: true` knob is a still-honoured back-compat **alias** — either key asserts unattended; new configs should prefer `autonomous.unattended`.)*

**Surfacing the sentry over your own CI.** The watchdog function itself — a job/sidecar that reads a run's liveness and consults the sentry classifier — already ships: the reference for an always-on substrate is the detached `sentry-poller`, which reads a run's heartbeat off shared filesystem and pages on a trip. To wire the same consult into your own CI, run `faff sentry check --json` (a machine-shaped verdict) from a watchdog job/sidecar on your own substrate, or `faff sentry check --summary-md FILE` for a markdown rendering suited to a job summary — the verb owns that rendering, so a local terminal readout and a CI summary never drift.

**Widening the poller's staleness window is a config key, not an env var.** The detached `sentry-poller` and `faff sentry check` read their heartbeat-staleness / no-progress window from `sentry.stall_window_secs` in `.faffrc` (default 900s). The environment variable `FAFF_RUN_HEARTBEAT_STALE_SECS` is a *different lane* — it belongs to the `runcheck`/`sentrycheck` Stop hooks and does **not** reach the poller. Forwarding it into the cage to make the poller tolerate a longer quiet period (say, a long producer subagent) is a no-op: the poller keeps aborting at the config window. So widen the poller by setting `sentry.stall_window_secs`, not the env var. faff makes the mistake loud — when `FAFF_RUN_HEARTBEAT_STALE_SECS` is set and `sentry.stall_window_secs` is unset, `faff sentry check` flags `stall_window_env_ignored: true` in its payload (it rides into the run's `events.jsonl` on the abort tick) and prints a warning naming the correct key, and `faff sentry-poller start` warns at arm time. (A related but separate failure mode — a long producer subagent that never ticks the parent heartbeat, so the heartbeat ages past the window even though the run is healthy — is not addressed by this warning; widen `sentry.stall_window_secs` to tolerate genuinely long quiet periods.)
...
**One ledger, minted once, reused end-to-end.** `faff lights-out` is the *only* mint for the whole run — you run it once, in the first cage, and hand the run dir it prints off to whatever drains next via `FAFF_RUN_DIR`. `faff-beep-boop` §0a — and, nested inside it, `/faff-plot --autonomous`'s Ignition — inherit that exact directory and **reuse** it: they never call `faff lights-out` again, so there is never a second `owner.status:"running"` ledger for the same run. What either stage *discovers* along the way (the PRD's creative-licence, the resolved root container) is written **onto** that one ledger through a narrow, lock-guarded field update (`faff run-record-prd`), never by minting again. A `FAFF_RUN_DIR` that names no readable L4 ledger fails the drain loudly rather than silently minting over it or quietly dropping to L3 — see the disposition contract above, which reads this exact reused ledger.
