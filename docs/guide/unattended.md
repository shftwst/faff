...
**Giving an unattended L3 run the Sentry kill-switch.** By default L3's Sentry is **advisory** — it logs a derailment for the morning review but does not *kill* the run live, because L3 assumes a human on the loop. That assumption breaks for a watcher nobody is watching overnight. Set `autonomous.sentry_acting: true` (in `.faffrc.yaml`) to let an unattended L3 run **abort** on a Sentry trip, exactly as L4 does — the kill-switch decoupled from the full L4 mandate. It is fail-safe off: only a literal `true` enables it, and a typo or unset value leaves the advisory default in force (a set value is echoed in the `faff config resolved` run banner so you can confirm it took). The knob covers the **abort** kill-switch only — the finer `pause`/`correct` interventions stay L4-only, so an L3 run with the knob gains the live safety stop without the adversarial steering. This is the one knob the **self-directed faff-on-faff watcher** most wants: that watcher *cannot* be L4 (L4 refuses a self-directed run), so before this it could never get a kill-switch however unattended it ran.
