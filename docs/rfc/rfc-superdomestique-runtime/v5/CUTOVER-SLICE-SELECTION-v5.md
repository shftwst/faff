...
| 3 | One canonical writer per state change | 2 | `faff effects declare` / `faff effects observe` — both reach disk through the single events.js :: appendRecordsUnderLock chokepoint; the map records this as clean "many callers, one writer", not a single-writer finding |
...
| 9 | Present assurance class and its gap | 1 | `faff effects declare` — journal J-C (chained-append producer binding) meets the governance floor, but the protected-effect class is **E-C** ("declare before acting … detection and reconciliation, never prevention"), below the E-B prevention target; the mechanism that raises it is named — the mediated protected-effect decision, facade verb 3, which is precisely this slice's own target verb |
...
- **U1 — Two facade "verbs" are compound operations.** Entries 4 ("append observations *and* request reconciliation") and 6 ("seal *and* export an audit bundle") each fuse two operations that fire at different times; reduced to atomic operations the facade has eight, not six (admit · register · decide · append · reconcile · rule · seal · export). **Routing:** FAFF-827 / the Phase-2A facade design. **Reason it is not settled here:** the facade's operation granularity is a design decision for the phase that implements it, not something the state-authority map or a slice choice fixes; it does not change which slice is picked first, but it changes how the facade is decomposed.
