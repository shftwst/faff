...
| `faff decision-capture` | translated | a Phase 2A reader treats an exported decision-capture record as frozen instrumentation history for that run | none stated — no safe-boundary condition names this row's fact | undefined; reversible (Phase 0/1 — no generic-format work item exists yet, so the irreversible boundary has not been crossed) | test/decision-capture.test.mjs |
