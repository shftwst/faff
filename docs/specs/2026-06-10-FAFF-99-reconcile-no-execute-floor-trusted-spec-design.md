...
> Build note (2026-06-10): the spec also listed `faff-workit/SKILL.md` as a consistency surface. That skill does **not** exist in this repo's `skills/` source (it is an installed-copy-only artifact), so the §6 faff-workit Punt is **moot for this build** and dropped from scope. The repo edits are the gateway floor + faff-graft Step 8 + the FAFF-8 comment update.
...
- **Chosen (resolved at build, 2026-06-10):** out of scope — `faff-workit` does not exist in this repo's `skills/` source (installed-copy-only artifact), so there is nothing in-repo to reconcile. The original Punt is moot for the repo build.
