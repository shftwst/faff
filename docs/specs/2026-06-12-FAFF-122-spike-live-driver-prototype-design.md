...
7. Analyse: flakiness (decision diff across runs), cost (mean+spread), fidelity (per approach,
   missed-seam classes). Write docs/adr/0003 with numbers + go/no-go + live-driver sizing.
...
- **Assumes:** ADR 0002's "assert at deterministic seams" framing is the capture target (buckets/ordering/verdicts/mutations). *Validation:* `docs/adr/0002-skill-test-architecture.md` (present).

## 7. DONE — Definition of Done

### The deliverable (a spike is done when the ADR exists)
- [ ] `docs/adr/0003-live-driver-spike.md` exists with: **flakiness**, **token cost**, and **seam-capture fidelity** numbers from real runs over the throwaway substrate.
