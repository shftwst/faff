...
5. **The committable artifact ships.** `docs/adr/0003-live-driver-spike.md` carries an addendum recording the routing live-driver runner extension + the baseline headline (or the blocked-on-auth note) + the runner location + a pointer to the gitignored `eval/report/` record.
