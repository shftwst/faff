...
6. **Emit the artifact** at `docs/audits/FAFF-114-skill-prompt-audit.md`, design-doc house style, skimmable. No `SKILL.md` edited.
...
- **Decision 1 — output location.** **Chosen:** a committed doc at `docs/audits/FAFF-114-skill-prompt-audit.md` (build agents run in worktrees off the committed repo; gitignored `design/` is invisible to them; tracker comments are unwieldy for a ~100-row inventory).
...
- **Assumes:** the committed `docs/` tree exists + `docs/audits/` is acceptable. *Validate:* `docs/specs/`, `docs/adr/` exist.
...
- [ ] Artifact emitted at `docs/audits/FAFF-114-skill-prompt-audit.md`, skimmable, no coded label scheme.
