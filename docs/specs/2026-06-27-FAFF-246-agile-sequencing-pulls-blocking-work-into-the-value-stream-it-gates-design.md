...
- [ ] `faff validate-adapters` passes (line caps, dedup, skimmable per docs/skill-authoring.md); no new `FAFF-NN`/`ADR` refs introduced into the edited `SKILL.md` (the spec's ref to FAFF-215/245 belongs in rationale/commit, not the executed prose — `faff lint-refs` must still pass).
