...
PROCEDURE faffter-noon-architecture(brief_or_spec):
  ...
  # proposes only — writes nothing under docs/adr/, makes NO `faff adr new` call
```
...
3. (assertion) Any proposer run makes **no `faff adr new`** call and writes nothing under `docs/adr/`; candidates appear only as `## ADR promotion intent`.
...
- [ ] Makes **no `faff adr new`** call; writes nothing under `docs/adr/`. *(Sc 3)*
