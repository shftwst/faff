...
A Nygard ADR in `docs/adr/`. The spec fixes the **decision + thesis** (the slot writes the Context/Decision/Consequences prose):
...
3. **Self-contained-prose guard.** `SKILL.md` files and `docs/guide/` executed prose carry **no** external tracker/ADR refs — `faff lint-refs` gates CI. Within-prose anchors are allowed. The ADR itself and other `docs/adr/*` files legitimately cite provenance.
...
- **The ADR records the decision** — list `docs/adr/` → a new Nygard ADR whose Decision states both theses.
...
1. An ADR is merged in `docs/adr/` whose Decision states **both** theses.
