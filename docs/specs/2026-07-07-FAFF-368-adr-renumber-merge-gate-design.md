...
| Term | Definition |
|---|---|
| Incoming ADR | The ADR file **added by this PR's diff** under `docs/adr/` (`git diff --diff-filter=A <merge-base>..HEAD -- docs/adr/`). The only file the guard may renumber. |
...
faff adr renumber <selector> --to <target> [--ref-scope <file>[,<file>...]] [--root <dir>]
  ...
  selector    a docs/adr filename (e.g. 0043-events-not-rpc.md) OR a bare number.
...
**Chosen:** `fs.renameSync` inside the CLI (consistent with `new`/`supersede`, which are pure-`fs`); no `git` invocation in the CLI. Graft stages the rename with `git add -A docs/adr/` and commits — git detects the rename by content similarity, so history is preserved without coupling the CLI to a git context (tests run in tmp dirs).
...
PROCEDURE adr_merge_guard(pr, branch, mergeTarget):
  1. added = git diff --diff-filter=A <merge-base(mergeTarget)>..HEAD -- docs/adr/
  ...
  5. FOR each "duplicate ADR number NNNN — f1, f2, …" problem:
     ...
     d. refScope = git diff --diff-filter=AM <merge-base>..HEAD -- docs/adr/
...
```
Given a PR whose diff adds no file under docs/adr/
...
**Chosen:** `git diff --diff-filter=A <merge-base>..HEAD -- docs/adr/` — the file this PR added. Robust under a duplicate number (identifies by *file*, not number).
...
- [ ] Step 10 guard is a no-op when the PR's diff adds no `docs/adr/` file (no validate-driven merge decision, no commit).
...
```
PROCEDURE smoke():
  1. tmp docs/adr with 0001..0003 clean; add 0003-dup.md (duplicate of 0003)
...
Premise-superseded scan over Done tickets on this surface (`faff adr` CLI, `docs/adr/`, faff-graft Step 10, `validate.yml`): **premise still holds — proceed.**
