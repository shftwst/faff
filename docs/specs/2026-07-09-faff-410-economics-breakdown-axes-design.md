...
- **Deleting `scripts/token-breakdown.mjs`.** — *Why excluded:* it is referenced by the FAFF-407 report (`docs/reports/token-usage-breakdown/`) as the spike artifact; the durable logic moving into the command does not require removing the artifact. — *Extension point:* a later cleanup pass may retire it once the report is archived.
