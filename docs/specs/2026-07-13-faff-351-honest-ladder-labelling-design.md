...
| `docs/skill-authoring.md` + `faff validate-adapters` | — | Lint gate the edited `SKILL.md` must still pass (line caps, no duplicated blocks) |
...
- **README / architecture-doc sweep of the same claims** — *Why excluded:* owned by peer **FAFF-432** (README/arch sweep). *Extension point:* `README.md` and `docs/architecture/*` carry their own copies of the ladder claims; 432 reconciles them against this table once it lands.
