...
| `docs/adr/0065-sentry-watchdog-invocation-locus-*.md` | ADR | The settled design this implements (probe-verified detach/sentinel/self-stop behaviour) |
...
- **Assumes:** ADR-0065 exists and stands as the governing design (status Proposed; its blocker spike FAFF-426 is Done and this issue is its named follow-up). Validation: `docs/adr/0065-sentry-watchdog-invocation-locus-*.md` present with the follow-up matching this issue's scope.
