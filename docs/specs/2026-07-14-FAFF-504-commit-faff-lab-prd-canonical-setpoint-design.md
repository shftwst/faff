...
|---|---|---|
| `docs/external-verification/README.md` | Markdown | Frames the SUT as a separate subject repo; establishes that P1–P5 keep the PRD *out* of the faff repo |
| `docs/external-verification/scaffold-p2-task-api.sh` | Bash | Heredocs its brief into the SUT; documents the git-only→tracker upgrade path; cites the non-existent `faff prd` commands |
| `docs/adr/` (0001–0066, next 0067) | Markdown | 66 Nygard-style ADRs; the format and next-number this ticket's ADR follows |
...
- **`scaffold-faff-lab.sh`** — the scaffold script itself.
  ...
  - **Extension point:** `docs/external-verification/scaffold-faff-lab.sh` (new file, FAFF-505), which will `cp` the PRD from the canonical doc this ticket creates.
...
- **Fixing the P2 scaffold's wrong runbook** — correcting `scaffold-p2-task-api.sh`'s citations of `faff prd new --from` / `faff prd admit`.
  ...
  - **Extension point:** FAFF-507, against `docs/external-verification/scaffold-p2-task-api.sh`.
...
| Canonical PRD doc | The single in-repo home for the PRD text: `docs/external-verification/faff-lab/PRD.md` |
...
```
FILE docs/external-verification/faff-lab/PRD.md      # NEW
...
The candidate path is `docs/external-verification/faff-lab/PRD.md`: colocated with the suite it belongs to, so FAFF-505's scaffold sits beside it.
...
- **The failure:** the recorded admission flow reintroduces `faff prd new --from` / `faff prd admit` (they're "obvious" and the P2 doc models them), so FAFF-505 is built against fiction again.
  - **How you'd know:** `grep -RnE 'faff prd (new --from|admit)' docs/external-verification/faff-lab/` returns matches.
...
When docs/external-verification/faff-lab/PRD.md is diffed against the ticket's PRD text
...
When `grep -RnE 'faff prd (new --from|admit)' docs/external-verification/faff-lab/` is run
...
- *Options:* (a) `docs/external-verification/faff-lab/PRD.md` — colocated real doc; (b) faff's own `docs/prd/<slug>.md`; (c) a heredoc inside the FAFF-505 scaffold (P1–P5 pattern).
...
- **Chosen:** `docs/external-verification/faff-lab/PRD.md` — a real, human-editable, diff-able doc colocated with the suite; the FAFF-505 scaffold copies from it, keeping the PRD singly-sourced and un-escaped. (decides: architecture)
...
- **Assumes:** `docs/adr/` next free number is 0067. *Validation:* `ls docs/adr/ | sort | tail` — confirm no `0067-*` exists before authoring; if the log advanced, use the true next number.
- **Assumes:** `docs/external-verification/faff-lab/` does not yet exist and the PRD is committed nowhere in-repo. *Validation:* `test -e docs/external-verification/faff-lab/PRD.md` and a repo-wide grep for a distinctive PRD phrase; if either hits, reconcile before writing.
...
- [ ] `grep -RnE 'faff prd (new --from|admit)' docs/external-verification/faff-lab/` returns zero matches
