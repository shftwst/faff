...
The five external-verification scaffolders in `docs/external-verification/scaffold-p{1..5}-*.sh` stand up throwaway SUT repos that faff is pointed *at* — the only miles the un-dogfoodable architecture→env→evaluate lanes ever get. Two of them (**P2** = `scaffold-p2-task-api.sh`, **P4** = `scaffold-p4-stripe-testmode.sh`) write RUNBOOKs whose PRD-leash-authoring steps **error on the current CLI**, so the acceptance run stalls at step 2 before any behaviour is instrumented. Concretely, both RUNBOOKs tell the operator to run:
...
2. **One canonical "how to author + admit a PRD leash" doc** under `docs/external-verification/`, referenced by both RUNBOOKs instead of inlining commands — so the next CLI drift is a one-file fix.
...
New `docs/external-verification/authoring-and-admitting-a-prd.md` (~1 screen): states the two bullets above, the exact current verb surface (`faff prd path|new|link|list|validate`; `faff prdr … admit|coverage|yagni`; the `prd-readiness` contract), and an explicit "what NOT to run (removed/never-existed): `prd new --from`, `prd admit`". P2+P4 RUNBOOK step-2 sections shrink to a one-line pointer plus the SUT-specific note (L4 auto-gate vs interactive).
...
- **S1 — P2 leash step no longer errors.** `grep -nE 'prd new --from|faff prd admit' docs/external-verification/scaffold-p2-task-api.sh` → nothing.
...
- **S8 — no regression in clean files.** `grep -rn 'price_per_mtok' docs/external-verification/` → nothing; P5 diff limited to the `gitignore-ensure` line.
...
1. `grep -rnE 'prd new --from|faff prd admit' docs/external-verification/` → **no matches**.
...
6. `docs/external-verification/authoring-and-admitting-a-prd.md` **exists**; P2 + P4 scaffolders each contain its basename.
7. `grep -rn 'price_per_mtok' docs/external-verification/` → **no matches**; P5 diff limited to the added `gitignore-ensure` line.
