...
Autonomous prep (appetite: high) records one architecturally-significant, cross-slice decision for `/faff-graft` to materialise via `faff adr new` on the feature branch (prep writes nothing under `docs/adr/`):
