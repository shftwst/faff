...
This spec is for the build agent that hardens the four external-verification SUT scaffolders (`docs/external-verification/scaffold-p{1..5}-*.sh`) so a freshly-scaffolded System-Under-Test can actually complete a lights-out (L4) run end-to-end. It also serves human reviewers deciding the one open posture question (where a SUT's L4 dials live). The scaffolders are dogfooding infrastructure: each stands up a throwaway repo that exercises faff against a non-faff product.
...
|---|---|---|
| `docs/external-verification/scaffold-p{1,2,3}-*.sh` | Bash | The L4-eligible scaffolders this change edits (dials, env, PRD, backend) |
| `docs/external-verification/scaffold-p{4,5}-*.sh` | Bash | Gated SUTs — no L4 dials; env/backend out of scope, PRD stays at root |
...
```
FAFF_ROOT = realpath( dirname(BASH_SOURCE[0]) / ".." / ".." )   # scaffolder lives in docs/external-verification/
...
```
PROCEDURE smoke():
  1. SUT_ROOT=/tmp/faff-sut-p1 bash docs/external-verification/scaffold-p1-link-shortener.sh
  2. cd $SUT_ROOT
