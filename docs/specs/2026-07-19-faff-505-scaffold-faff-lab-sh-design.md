...
- **The PRD is singly-sourced and immutable.** The committed `docs/external-verification/faff-lab/PRD.md` is *the* setpoint. The scaffold copies it verbatim; it never re-authors or edits it, and the run never edits it.
...
|---|---|---|
| `docs/external-verification/scaffold-p2-task-api.sh` | bash | The template this mirrors exactly (PRD-driven rung, git-only, adversarial dials) |
| `docs/external-verification/scaffold-p3-landing-page.sh` | bash | Deploy-shaped precedent; `env: faffter-noon-env-compose` local stand-in |
| `docs/external-verification/faff-lab/PRD.md` | markdown | The committed setpoint the scaffold copies verbatim |
| `docs/adr/0070-faff-lab-tracker-vs-git-only.md` | markdown | The tracker decision (git-only-first) the `.faffrc.yaml` must reflect |
| `docs/external-verification/faff-lab/README.md` | markdown | Already documents the real admission flow the RUNBOOK reuses |
| `docs/external-verification/authoring-and-admitting-a-prd.md` | markdown | The real `faff prd` / `faff prdr` / `prd-readiness` verb surface |
| `docs/external-verification/README.md` | markdown | The suite index whose "five rungs" framing becomes six |
...
- **Re-authoring or editing the faff-lab PRD.** — The PRD is committed and immutable (FAFF-504, ADR 0070). Extension point: `docs/external-verification/faff-lab/PRD.md` is edited only by a human, never here.
...
```
SUT_ROOT=<target dir> bash docs/external-verification/scaffold-faff-lab.sh
```
...
PROCEDURE scaffold_faff_lab:
  ...
  7. mkdir -p docs/prd; cp "$FAFF_ROOT/docs/external-verification/faff-lab/PRD.md" docs/prd/faff-lab.md
...
- **Assumes:** `docs/external-verification/faff-lab/PRD.md`, `docs/external-verification/faff-lab/README.md`, and `docs/adr/0070-faff-lab-tracker-vs-git-only.md` are present and committed (FAFF-504). Validate: `test -f` each before the `cp`; the copy step depends on the PRD existing at that path.
...
### From WHY
- [ ] `docs/external-verification/scaffold-faff-lab.sh` exists, is executable, and mirrors P2's structural conventions (SUT_ROOT, FORCE refusal, FAFF_ROOT-before-cd, git init, secret-leak guard, hooks-ensure, initial commit).
...
- [ ] `docs/prd/faff-lab.md` is a verbatim copy of `docs/external-verification/faff-lab/PRD.md` (`diff` empty), not a heredoc.
...
- [ ] `docs/external-verification/README.md`'s "five rungs" framing becomes six, with a table row for `scaffold-faff-lab.sh` noting faff-lab is a permanent deliverable unlike P1–P5.
...
PROCEDURE smoke:
  ...
  2. SUT_ROOT=$SUT bash docs/external-verification/scaffold-faff-lab.sh
  3. ASSERT files exist: $SUT/{.faffrc.yaml,BRIEF.md,RUNBOOK.md,docs/prd/faff-lab.md}
  4. ASSERT diff $SUT/docs/prd/faff-lab.md docs/external-verification/faff-lab/PRD.md is empty
