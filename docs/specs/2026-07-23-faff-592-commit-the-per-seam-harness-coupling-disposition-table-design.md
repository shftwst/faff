...
| `plugin/skills/faff/bin/lib/merge-fence.js`, `docs/adr/0043-*.md` | Node + ADR | The PreToolUse merge fence and the forge-side floor it moves down to |
...
**Scope statement:** this is the deliverable half of the FAFF-477 spike, committed under `docs/architecture/` alongside the existing L3/L4 architecture doc, and the starting inventory for FAFF-483's interface definition.
...
- **Restructuring existing docs** — why excluded: `docs/architecture/` already exists with the L3/L4 doc; this adds one file, moves nothing.
...
PROCEDURE commit_disposition_table:
  1. Create docs/architecture/harness-coupling.md
...
Given `docs/architecture/harness-coupling.md` is committed on main
...
**Where does the artifact land?** FAFF-477 originally named `design/harness-portability-surface.md`; no `design/` directory exists in the repo, and the ticket that now owns the deliverable names `docs/architecture/harness-coupling.md`, beside the existing L3/L4 architecture doc. **Chosen:** `docs/architecture/harness-coupling.md` — the FAFF-592 path supersedes the spike's provisional path; the human comment on FAFF-477 confirms the deliverable moved.
...
**Assumes:** the evidence anchors exist as verified at prep time — `plugin/skills/faff/bin/lib/{budget,hooks-ensure,governance-check,merge-fence,engine,backends}.js`, `plugin/skills/faff-graft/setup-worktree.sh`, `docs/adr/0043-*.md`, and the Stop/PreToolUse hook lists (`runcheck`/`prepcheck`/`sentrycheck`, `merge-fence`/`background-fence`). Validation: before writing each row, confirm the named path or list exists; if one has drifted, update the row's anchor to the current location rather than parking.

## 8. DONE — Definition of Done

### From WHY
- [ ] `docs/architecture/harness-coupling.md` exists on the feature branch and states the load-bearing principle exactly once, in the opener
