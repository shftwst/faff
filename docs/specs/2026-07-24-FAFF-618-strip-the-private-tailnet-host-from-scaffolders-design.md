...
|---|---|---|
| `docs/external-verification/scaffold-faff-lab.sh` | bash | Host at ~:113; `.gitignore` heredoc at :46; no `gitignore-ensure` (commits `.faff/`) |
| `docs/external-verification/scaffold-p1-link-shortener.sh` | bash | Host at ~:90; calls `gitignore-ensure` at :223 |
| `docs/external-verification/scaffold-p2-task-api.sh` | bash | Host at ~:92; refs list at :100–103 |
| `docs/external-verification/scaffold-p3-landing-page.sh` | bash | Host at ~:84; refs list at :92–95 |
...
- **The host in test fixtures and frozen design specs.** — Excluded, exactly as FAFF-587 carved out: `test/*.mjs`, `plugin/skills/faff/bin/lib/backends.js`, and the frozen `docs/specs/*.md` use the host as a tailnet-detection fixture or a historical record. The acceptance grep is scoped to `docs/external-verification/*.sh`, so these are untouched. **Extension point:** a separate hygiene ticket could swap the fixtures to a generic `*.ts.net` placeholder that preserves the suffix-detection assertions.
...
- **The failure:** the host survives in a second heredoc in the same file (a RUNBOOK or BRIEF block) so the leak-census still hits. **How you'd know:** `git grep longhair-escalator docs/external-verification/*.sh` returns a hit after the edits. **What it means:** not done — the grep is the acceptance; every occurrence in the four files must go (grep confirms today the only occurrence per file is the `host:` line, so this is a guard, not an expected extra edit).
...
Then `git grep longhair-escalator docs/external-verification/*.sh` returns no matches
```
...
- **The host appears once per file, on the `ollama-local.host` line.** Validate: `git grep -n longhair-escalator docs/external-verification/*.sh` before editing — confirmed four hits, one per file.

## 8. DONE — Definition of Done

### From WHY / info-leak
- [ ] `git grep longhair-escalator docs/external-verification/*.sh` returns zero matches.
...
```
1. On a clean clone: git grep longhair-escalator docs/external-verification/*.sh          → no output
2. FAFF_EVAL_LOCAL_BASE_URL=http://example-host:11434 SUT_ROOT=/tmp/sut-p2 \
     bash docs/external-verification/scaffold-p2-task-api.sh
   then in /tmp/sut-p2:
