...
|---|---|---|
| `docs/architecture/harness-coupling.md` | Markdown | The inventory this change edits. Eight rows, a four-term disposition vocabulary, a closing "how to extend" rule. |
...
| `docs/adr/0090-engine-transport-gains-a-spawn-family-codex-extending-adr-0054-s-per-lane-transp.md` | ADR | Frames the read-only posture as *"The child gets no hands"* — least privilege on a pure-data-in producer, not a blast-radius boundary. |
...
Each lands in the existing seam table in `docs/architecture/harness-coupling.md`, in the same four-column shape (Seam / Today (Claude Code) / Disposition / Evidence and follow-on) the eight committed rows use.
...
PROCEDURE extend_harness_coupling_inventory:
  1. Read docs/architecture/harness-coupling.md in full — the four-term
...
**Anti-pattern:** creating `design/harness-agnostic-runtime.md` because the ticket says so. Why: no `design/` root exists, FAFF-592 already redirected the equivalent path into `docs/architecture/`, and a second inventory breaks the tracing rule FAFF-483 depends on.
...
```
Given docs/architecture/harness-coupling.md on main after this change
...
- No new file is created in `design/`, and this change adds no seam classification anywhere outside `docs/architecture/harness-coupling.md`.
...
Options: the ticket's `design/harness-agnostic-runtime.md`; a new page under `docs/architecture/`; an edit to the existing `docs/architecture/harness-coupling.md`. The ticket's path predates FAFF-592, which redirected FAFF-477's equivalent provisional `design/` path into `docs/architecture/` and never created a `design/` root. A third option would leave the repo with one documentation root for eight seams and another for five.
**Chosen:** edit `docs/architecture/harness-coupling.md` — the ticket's path is stale, and the doc's own extension rule already anticipates added rows.
...
- [ ] This change creates no second harness-seam inventory: every row and detail subsection it adds lands in `docs/architecture/harness-coupling.md`, and it creates no other page that classifies harness seams. (Pre-existing pages that mention the disposition terms in passing — `docs/reports/tracker-filing-plan.md` quotes them in a filed ticket body — are untouched and are not a second inventory.)
...
PROCEDURE cold_read_smoke:
  ...
  1. Open docs/architecture/harness-coupling.md.
