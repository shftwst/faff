...
`gradeCoverage` (`eval/grader.mjs:429`) builds a boolean vector: one entry per `must_include` synonym set, then one per `must_avoid` set. For `grouping-001` that is four plus two, six entries. Naming them the way the code comment beside the `grouping` arm does, so no index arithmetic is needed: the **password-reset set** and the **invoicing set** are ordinary domain vocabulary reachable from the fixture's own ticket titles; the **leave-loose set** and the **ordering-edge set** are faff's own idiolect — "leave loose" / "leave-loose" and "coherence edge" / "sequencable". "Sequencable" is a misspelling that occurs nowhere outside faff's prose. Then the two `must_avoid` sets: the **umbrella-label set** ("tech debt", "chores", "miscellaneous", "housekeeping", "grab bag") and the **layer-label set** ("backend work", "frontend work", "infra work", "refactors", "the auth layer"). With the idiolect phrases removed from the question, the `--no-plugin` control has no in-prompt source for the leave-loose or ordering-edge sets at all, and can only reach them by chance.

So the expected control vector is `[true, true, false, false, true, true]` — four of six, a score of **0.667** — and the expected `--plugin` delta on this kind is carried substantially by vocabulary the rubric supplies rather than by judgement quality. Two entries of six on this case measure whether the model adopted faff's words. That is a known limitation of a synonym-coverage oracle and it is not this ticket's to fix; what this ticket owes is that nobody reads a low control score as a regression or reads the plugin delta as pure judgement gain.

> **Superseded on four of the six positions.** The single figure above does not hold: the ordering-edge set is reachable from the fixture's own `dependency_graph` keys, the password-reset set turns on a hyphen the grader does not fold, and both `must_avoid` sets are contingent on the control's own word choice rather than settled. The honest expectation is a band with a conditional lower bound, not a number. The corrected reasoning is in §4's failure modes, "The recorded `grouping` control floor was written as a derivation when it is an estimate", and that is the version the sweep should be read against.

**Chosen:** record the expected control **band** rather than a single floor — 0.5 to 0.833 while both `must_avoid` positions hold, shaped `[?, true, false, ?, true, true]`, dropping to 0.333 if either `must_avoid` position flips — together with each open position and the reason it is open, as an interpretation note on the ticket and in a comment beside the `grouping` arm. And add a DONE item asserting that the reworded question is not the only in-prompt source of any oracle set — mechanically, that for each of the four `must_include` sets, at least one member is reachable from either the fixture or the loaded criteria prose, so no set is unreachable-by-construction under `--plugin`. Rejected: rewriting the oracle to drop the leave-loose and ordering-edge sets, which would edit the oracle this spec has pinned byte-identical, days before a paid sweep, and would silently change what `grouping` means.
...
**The recorded `grouping` control floor was written as a derivation when it is an estimate — corrected here.** §3 above ("What the rewording does to `grouping`'s first honest number") recorded the expected `--no-plugin` control as the vector `[true, true, false, false, true, true]` and the score **0.667**, on the reasoning that the leave-loose and ordering-edge sets are both faff idiolect with no in-prompt source, and that both `must_avoid` positions were settled. Three of those four judgements do not survive contact with the fixture, and the single number only looks right because two of the errors point in opposite directions and cancel.

- **The ordering-edge set is reachable from the fixture — and is closer to likely-true than to open.** `dependency_graph` serialises into the prompt as `[{"blocker":…,"blocked":…}]`, and `gradeCoverage` matches through `entryMatches` — a plain lowercase substring test with no folding at all. A control answer that echoes the key name `blocker` in an ordering-edge line scores the set true with no rubric involved, and `blocked by` — the other natural phrasing of the same line — is a synonym in the same set. "Sequencable" occurring nowhere outside faff's prose says nothing about the set, which passes on any one of its synonyms. It is carried below as unsettled only because hedging in that direction is the cheaper mistake.
- **The password-reset set depends on a fold the grader does not do.** The fixture only ever writes `password-reset`; the oracle wants `password reset`. `entryMatches` does not fold hyphens — the grader's `normLabel`, which does fold, serves `gradeSplittable` and never touches `grouping` — so the set scores true only if the model happens to write the phrase unhyphenated, or reaches "account recovery" on its own. This one is genuinely open.
- **Neither `must_avoid` set is settled on the control arm.** `gradeCoverage` scores `must_avoid` against the model's *own answer text*. The question asks for a one-line reason per unplaced ticket, and `TCK-31` — "Bump the CI runner base image to ubuntu-24.04" — is the ticket with no outcome partner, so the control writes such a line for it. "Housekeeping", "chores" and "infra work" are exactly the words a model reaches for describing an orphaned CI chore, and they sit in the umbrella-label and layer-label sets respectively. What suppresses that vocabulary is the leave-loose rubric — and the control is the arm that does not load it. On the `--plugin` arm these two positions are safe; on the control they are the weakest assumption in the vector, not the strongest.

So only the invoicing set is safely true, and only the leave-loose set is provably beyond the control's reach. The honest expectation is a **band of 0.5 to 0.833**, shaped `[?, true, false, ?, true, true]` — and that lower bound is **conditional on both `must_avoid` positions holding**, not a floor. One `must_avoid` flip drops the control to 0.333 with both open `must_include` positions still open, which is below the band and still not a regression. 0.667 sits in the middle of the band by coincidence, not by derivation. *How you would know:* not from the score, which is the whole problem — a hard floor recorded here would be worse than none, because it invites the next reader to treat an ordinary result as a finding or a genuine slip as expected. *What it means:* for FAFF-614, read the per-case signature vector rather than the score. On the control arm, diagnose a `must_avoid` flip as a missing-rubric effect first — naive labelling is the very behaviour that set was written to catch — and as a task shift only if the same position also flips on the `--plugin` arm, which does load the rubric. That contrast between the arms is the diagnostic, not the flip on its own. The invoicing set going false on either arm is the one unambiguous sign that the task moved.

**The `grouping` rewording could change the task, not just the vocabulary — and the signature is a vector, not a shape change.** Revision 1 said you would notice this because "the model's answers stop containing membership maps or stop accounting for every ticket — a structural change in the output shape". That signal is wrong for the thing it is meant to catch. `gradeCoverage` scores by synonym-set coverage; a collapse on the leave-loose and ordering-edge sets leaves output shape entirely intact — a perfectly structured answer with every container, every edge and every unplaced ticket accounted for still scores inside the control band. The correct signal is the vector itself, which the grader already writes to the signature field as `JSON.stringify(vector)`. *How you would know:* read the per-case signature, not the transcript's shape. A `--plugin` vector with the leave-loose set false means the rubric is failing to supply the idiolect and the delta this eval measures has gone; a vector degrading on the invoicing set, or a `must_avoid` position flipping on the `--plugin` arm as well as the control, means the task genuinely moved. *What it means:* a control anywhere in the band recorded above with the leave-loose set false is expected, not a finding, and so is a control at 0.333 whose only extra loss is a `must_avoid` position the `--plugin` arm still holds; anything else is narrow — re-word once more against the must-still-do list; the oracle stays put either way.
...
- [ ] The expected `--no-plugin` control for `grouping-001` — the band 0.5 to 0.833 while both `must_avoid` positions hold, 0.333 if either flips, shaped `[?, true, false, ?, true, true]`, with every open position, its reason, and the arm-contrast reading for a `must_avoid` flip — is recorded as an interpretation note on the ticket, in a comment beside the `grouping` arm, and in §4's failure modes.
