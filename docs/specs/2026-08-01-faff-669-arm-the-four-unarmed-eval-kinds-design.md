...
> **Superseded on two of the six positions.** The single figure above does not hold: set 4 is reachable from the fixture's own `dependency_graph` keys, and set 1 turns on a hyphen the grader does not fold. The honest expectation is a band, not a number. The corrected reasoning is in §4's failure modes, "The recorded `grouping` control floor was written as a derivation when it is an estimate", and that is the version the sweep should be read against.
...
**The recorded `grouping` control floor was written as a derivation when it is an estimate — corrected here.** §3 above ("What the rewording does to `grouping`'s first honest number") recorded the expected `--no-plugin` control as the vector `[true, true, false, false, true, true]` and the score **0.667**, on the reasoning that sets 3 and 4 are both faff idiolect with no in-prompt source. That reasoning is wrong in one position and incomplete in another, and the single number only looks right because the two errors point in opposite directions and cancel.

- **Set 4 is reachable from the fixture.** `dependency_graph` serialises into the prompt as `[{"blocker":…,"blocked":…}]`, and `gradeCoverage` matches through `entryMatches` — a plain lowercase substring test with no folding at all. A control answer that echoes the key name `blocker` in an ordering-edge line scores set 4 true with no rubric involved. "Sequencable" occurring nowhere outside faff's prose says nothing about the set, which passes on any one of its synonyms.
- **Set 1 depends on a fold the grader does not do.** The fixture only ever writes `password-reset`; the oracle wants `password reset`. `entryMatches` does not fold hyphens — the grader's `normLabel`, which does fold, serves `gradeSplittable` and never touches `grouping` — so set 1 scores true only if the model happens to write the phrase unhyphenated, or reaches "account recovery" on its own.

So two of the six positions are genuinely uncertain, in opposite directions. Only set 3 (leave-loose) is provably beyond the control's reach; sets 2 and both `must_avoid` positions are the only safely-true ones. The honest expectation is a **band of 0.5 to 0.833**, shaped `[?, true, false, ?, true, true]`. 0.667 sits in the middle of that band by coincidence, not by derivation. *How you would know:* not from the score, which is the whole problem — a wrongly-recorded floor is worse than an unrecorded one, because it invites the next reader to treat an ordinary result as a finding or a genuine slip as expected. *What it means:* for FAFF-614, read the per-case signature vector rather than the score; treat anything inside that band as the control floor rather than a regression; treat set 2 going false, or either `must_avoid` position flipping, as the real signal that the task moved.

**The `grouping` rewording could change the task, not just the vocabulary — and the signature is a vector, not a shape change.** Revision 1 said you would notice this because "the model's answers stop containing membership maps or stop accounting for every ticket — a structural change in the output shape". That signal is wrong for the thing it is meant to catch. `gradeCoverage` scores by synonym-set coverage; a collapse on sets 3 and 4 leaves output shape entirely intact — a perfectly structured answer with every container, every edge and every unplaced ticket accounted for still scores inside the control band. The correct signal is the vector itself, which the grader already writes to the signature field as `JSON.stringify(vector)`. *How you would know:* read the per-case signature, not the transcript's shape. A `--plugin` vector with position 3 false means the rubric is failing to supply the idiolect and the delta this eval measures has gone; a vector degrading in positions 1 or 2, or a `must_avoid` position flipping to false, means the task genuinely moved. *What it means:* a control anywhere in the band recorded above, with position 3 false, is expected, not a finding; anything else is narrow — re-word once more against the must-still-do list; the oracle stays put either way.
...
- [ ] The expected `--no-plugin` control for `grouping-001` — the band 0.5 to 0.833, shaped `[?, true, false, ?, true, true]`, with the two open positions and their reasons — is recorded as an interpretation note on the ticket, in a comment beside the `grouping` arm, and in §4's failure modes.
