...
**The evidence is now on record (this is what unblocked the ADR).** FAFF-654's probe measured all four job-surface columns into `docs/spikes/2026-07-26-FAFF-654/RESULTS.md` (hosted-direct/container by FAFF-657, self-hosted-direct/container by FAFF-656). Three findings are load-bearing here:
...
- An ADR under `docs/adr/` stating the boundary requirement for a faff run on a CI runner, **per level**, with **no product in the normative part** (a named example only in a clearly non-normative illustration).
...
- The ADR cites the FAFF-654 record (`docs/spikes/2026-07-26-FAFF-654/RESULTS.md`) and `.github/workflows/validate.yml`'s `env-rootless` job as the in-repo hosted-direct demonstration.
...
- [ ] ADR under `docs/adr/` records the two admission criteria (contained; no host engine socket reachable), product-neutral in the normative part.
