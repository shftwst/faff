...
- A reference workflow committed under `docs/ci/` (e.g. `docs/ci/l3-watcher.yml`) that: runs on a schedule (`on: schedule` + `workflow_dispatch` for manual firing), invokes `/faff-beep-boop` at L3 (plain invocation, no `faff lights-out`), and targets this repo.
...
- A companion doc (under `docs/guide/` or alongside in `docs/ci/`) explains the workflow, the live-activation step (promote to `.github/workflows/` once the FAFF-609 rig + secret exist), and the product-neutral cage posture.
...
- No change to faff CLI behaviour; `node --test` stays green. If a docs/workflow lint exists (e.g. actionlint over `docs/ci/`), the reference workflow passes it.
...
- [ ] `docs/ci/<name>.yml` reference workflow: `on: schedule` + `workflow_dispatch`, L3 `/faff-beep-boop`, this repo.
...
- [ ] `node --test` green; reference workflow passes any docs/ci lint that exists.
