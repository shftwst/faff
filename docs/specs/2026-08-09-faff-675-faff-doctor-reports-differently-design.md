...
**Do not change the exit-code contract for the dev-linked path.** The gateway preamble, the FAFF-200 self-heal offer, and ten `--target` tests all depend on today's exit semantics for the global/dev-linked case (`docs/specs/2026-06-21-FAFF-200-...`, `test/doctor.test.mjs`). This change adds a plugin-root branch; it must leave every non-plugin-root verdict byte-for-byte as it is.
...
| `docs/adr/` | Markdown | ADR home; highest is `0095`. The plugin-vs-global answer lands as `0096`. |
...
### From decisions
- [ ] `docs/adr/0096-*.md` records the plugin-root-is-an-alternative decision, names FAFF-685 as the revisit trigger, and a comment at the `pluginRootEnv` branch of `resolveDoctorScanSet` cross-references it.
