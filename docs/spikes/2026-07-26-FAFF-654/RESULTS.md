...
notes: A self-test was run during the build on the author's machine at euid 501, reporting 11 assertions across 10 fixture cases, zero skipped and zero failed. That run is not committed here because local-dev is not one of the four shapes; the committed self-test outputs come from the dispatched jobs. It is recorded because the non-root demonstration criterion turns on at least one committed non-root run, and none is committed here. FAFF-657 owes it: its hosted-direct job is expected to run as a non-root user and to supply the first committed self-test output with zero skipped cases. Until it does, the portability guarantee this instrument offers FAFF-656 rests on an uncommitted local run.
