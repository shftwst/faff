...
The full frontier reverify is a ~1,900–4,750-run, multi-hour sweep — right for a substantive change, disproportionate for a small prose diff (so it gets waived). `--gate` makes the gate **proportionate** and **safe to run anywhere** via a selectable driver:
...
| File | Role |
|---|---|
| `cases/*.json` | `EvalCase` fixtures + human oracles (95 files across 32 kinds) |
...
> ⚠ Needs a real `claude -p` + (for frontier) a budget. 95 cases × K=20 base ≈ 1,900 reps, escalating toward 4,750 on wobbly cases.
```sh
...
After a calibration pass edits oracles (see `eval/calibration/oracle-triage.json`), 16 of the 32 kinds in
...
4. **What it costs (derived, not guessed).** At the current **95** live case files × **20** base reps
   ≈ **1,900** frontier reps; wobbly cases escalate toward **50** reps each, so the worst case is
   ≈ **4,750** reps — a **multi-hour**, real-dollars sweep. Budget for it; don't start it on a laptop
