...
The full frontier reverify is a ~1,820–4,550-run, multi-hour sweep — right for a substantive change, disproportionate for a small prose diff (so it gets waived). `--gate` makes the gate **proportionate** and **safe to run anywhere** via a selectable driver:
...
| File | Role |
|---|---|
| `cases/*.json` | `EvalCase` fixtures + human oracles (91 files across 31 kinds) |
...
> ⚠ Needs a real `claude -p` + (for frontier) a budget. 91 cases × K=20 base ≈ 1,820 reps, escalating toward 4,550 on wobbly cases.
```sh
...
After a calibration pass edits oracles (see `eval/calibration/oracle-triage.json`), 15 of the 31 kinds in
...
4. **What it costs (derived, not guessed).** At the current **91** live case files × **20** base reps
   ≈ **1,820** frontier reps; wobbly cases escalate toward **50** reps each, so the worst case is
   ≈ **4,550** reps — a **multi-hour**, real-dollars sweep. Budget for it; don't start it on a laptop
