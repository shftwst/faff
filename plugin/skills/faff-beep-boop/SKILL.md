...
- **At run start** (run-id minted, before step 4): write `owner` with `status:"running"`, `pid`, `session_id:<run-id>`, `started_at:now`, `last_heartbeat:now`, **and export the per-session pointer** so this session's own Stop hook recognises its own run:
  ...
  **Resolve the budget envelope and baseline tokens here too** (FAFF-36): resolve the `BudgetEnvelope` from the `budget:` config block + `--until`/`--max` flags, and capture the run-start transcript-sum as `tokens_at_start`, writing both into the ledger `budget` block before step 4. The cleanest way is one `faff budget check --root "$measure_root" --session-id "$session_id"` call at run start (its `spent.tokens` against an empty baseline is the start-of-run sum); record `envelope` + `tokens_at_start` so every later check measures *this run's* delta. **Also capture `budget.measure_root=$PWD`** (FAFF-488, before any worktree work) **and `session_id=$CLAUDE_CODE_SESSION_ID`** (already stamped into every `BuildDispatch.session_id`): pass both as `--root "$measure_root" --session-id "$session_id"` on every later token call, so an Agent-thread cwd reset never mis-resolves the transcript. **(FAFF-408) If you token-tag events**, also seed `budget.tokens_at_start_by_class = {input:0, output:0, cache_write:0, cache_read:0}` here (the four-class baseline the first `--tokens` event's checkpoint advances from); a stale/absent value degrades safely (the CLI falls back to zeros). Optionally also record `budget.tokens_at_start_by_model_class` (per-model, from the same run-start measurement) so a `budget.cost` ceiling under map pricing subtracts a real baseline instead of falling back to its pro-rated estimate — omitting it is safe (the CLI degrades and warns, never silently wrong).
