...
5. **Wave-boundary stop.** How a wave's empty (or all-parked) build queue terminates the run depends on the run's **level** and whether `--converge` is set:
   - **Plain L3, no `--converge` (the long-standing default, unchanged):** queue empty → exit to reporting; else run steps 5–7 again (conflict analysis → build pass → wave drain) and return to step 8 — termination by queue emptiness, no iteration cap.
...
- **Always wave-loops until queue stability.** The build phase is wave-based — after each wave's queue drains, the orchestrator re-assembles to pick up work unlocked by issues that just shipped. In a plain L3 run the run ends only when a wave assembles to an empty build queue: termination is by emptiness, with no iteration cap. A **lights-out-minted (L4) run terminates by doneness** — it consults `faff run-done` at run-end (with or without `--converge`), so an emptied queue with an unsatisfied PRD or an escalated budget surfaces needs-human rather than reporting "complete". Under **`--converge`** the loop additionally folds in freshly-filed execution-discovered scope and applies the non-convergence backstop — still no fixed iteration count as the primary mechanism; the hard cap (`convergence.max_waves`) only ever fires as a *reported* runaway backstop.
