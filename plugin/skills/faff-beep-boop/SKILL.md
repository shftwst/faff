...
Filed tickets are **new work, not admitted issues** — they sit outside runcheck's `admitted − outcomes` invariant and never affect run completeness. Once an eligible discovered ticket is admitted to a wave's build queue (under `--converge` — see _Within-run convergence_ in Budget flags and the run-done stop at step 8.5), it **is** an admitted issue and gets a terminal outcome like any other, so runcheck's invariant covers it from that point.
...
- (**under `--converge`, or at L4 always, or (FAFF-559) any git-only run**) `faff run-done` decides the wave-boundary stop (step 8.5): `run-complete` → `Stop reason: converged/both-dry`; `escalate` → `Stop reason: non-convergence` (runaway backstop) / `budget-escalated(<dims>)` / `product-incomplete`, with the structured needs-human signal. A lights-out (L4) run consults it at run-end **even without `--converge`** — doneness, not bare queue-emptiness, terminates it; a **tracker-mode** plain L3 run without `--converge` ends by queue-emptiness and never consults it. A **git-only** plain L3 run has no tracker-observed emptiness to exit on, so it consults `faff run-done` at every wave boundary regardless of `--converge` (see step 8.5 → _Git-only signal assembly_).
...
- **Always wave-loops until queue stability.** The build phase is wave-based — after each wave's queue drains, the orchestrator re-assembles to pick up work unlocked by issues that just shipped. In a **tracker-mode** plain L3 run the run ends only when a wave assembles to an empty build queue: termination is by emptiness, with no iteration cap. A **lights-out-minted (L4) run terminates by doneness** — it consults `faff run-done` at run-end (with or without `--converge`), so an emptied queue with an unsatisfied PRD or an escalated budget surfaces needs-human rather than reporting "complete". Under **`--converge`** the loop additionally folds in freshly-filed execution-discovered scope and applies the non-convergence backstop — still no fixed iteration count as the primary mechanism; the hard cap (`convergence.max_waves`) only ever fires as a *reported* runaway backstop. A **git-only** plain L3 run has no tracker-observed emptiness to terminate on, so (FAFF-559) it too terminates by doneness — it consults `faff run-done` at every wave boundary regardless of level or `--converge` (step 8.5 → _Git-only signal assembly_), never by bare queue emptiness.
