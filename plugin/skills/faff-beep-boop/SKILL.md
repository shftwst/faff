...
All forms also accept two optional **cost-budget flags** that can be combined; the run stops when either fires. See `## Budget flags` below — which also documents the **`--converge`** / **`--no-converge`** flags (within-run convergence — drain execution-discovered scope in the *same* run; on by default, `--no-converge` opts a single L3 run out).
...
**Steps 1–5 assemble the seven `RunTriggerSignals` `faff run-start` (FAFF-496, shipped) consumes — reusing this section's own reads wherever possible, adding only the two genuinely new reads (outward, coverage). No ladder logic is re-derived here; `faff run-start` owns the `{verdict, reason}` decision.**
...
3. **Resolve `TargetRef {container, repo}` and `SelfRef` — the outward read (FAFF-521, new).** Mirrors `/faff-plot --autonomous` ignition's resolution **identically**, so both call sites populate `--self` via the same repo-slug match (gateway → *Sibling-skill invocation* conventions do not apply here — this is a direct CLI read, not a slot):
   ...
   - `SelfRef`: `self.repo := ` the same `tracking.repo` read above; `self.container := faff config get tracking.container` (the same read as the container fallback above — null by ADR-0069 construction on faff's own install); `self.is_self := target.repo != null AND target.repo == self.repo` (the repo-slug oracle).
...
4. Re-run step 4 (build queue assembly). The Todo set now includes any items prep just promoted. Re-assembly inherits step 4's **at-the-gate** discipline (gateway → **Re-ground before gate**): the labels + status feeding `faff next` / `faff eligible` and the claim-before-admit re-read are sourced **live at this re-entry**, never from the wave-1 read — so an issue a human cranked up *between waves* is correctly judged eligible on re-entry, not on its stale pre-crank labels. Re-running step 4 also **re-assembles the FAFF-535 `prd-distance` input** for this wave boundary (one fresh `faff prdr distance` call per wave under a resolved target PRD) — so as siblings advance toward `met` mid-run the ordering re-heads for the now-nearest PRDR; no target PRD in scope → no call, byte-identical to today.
5. **Wave-boundary stop.** How a wave's empty (or all-parked) build queue terminates the run depends on the run's **level**, whether the run is **opted out** of convergence (`--no-converge`, or `convergence.enabled: false` with no `--converge`), and (FAFF-559) whether the run is git-only. **`--converge` and `--no-converge` are mutually exclusive — passing both is a refused usage error, announced at run start, never silently resolved to one;** a flag overrides `convergence.enabled`, and at L4 both flag and knob are **inert** (the loop is forced on):
...
5. **File** each surviving item per the `faff-chain-gap-fill` recipe (see `/faff-tidy` → _Chain gaps_ — do not restate it): status `Backlog`, tag `faff-chain-gap-fill` via `faff label add <issue> faff-chain-gap-fill` and its descriptor's write (gateway → **Control-label provisioning**), the recorded relationship link to the originating issue, and a "discovered during build of SHF-XX" provenance line in the description. **Stamp `initiated: autonomous`** on the create — `faff intake-record <new-issue> --via jot --initiated autonomous` (FAFF-220; the marker is the autonomous-filing audit trail). **FAFF-536 self-intake exception:** an item classified `outward-self-intake` files to the **intake bucket** instead — status `Backlog`, label **`faff-jot-intake`** (NOT `faff-chain-gap-fill` — this is a defect record, not a chain gap), no `faff-automate`; same `--via jot --initiated autonomous` stamp; relationship link only when the entry names one. On an opted-out L3 run (file-and-defer timing) the next run's tidy + prep pass picks them up — depth grows one layer per run; under the convergence loop (the default; forced at L4 — filed at step 8.0) the *same* run's wave re-entry picks up the eligible ones — depth grows many layers per run, bounded by `faff run-done`.
6. **Outward-new-root (surface-only, never filed).** For each `outward` item: keep/record its `DiscoveredScopeEntry { containment: "outward-new-root" }`, **create nothing**, surface it in the run summary's discovered-scope section so `/faff-wtf` §4 shows it, **and post a comment on the mandate issue** (the `<ISSUE>` it came from — an already-sanctioned surface, so the comment is in-lane, not scope expansion). **Orchestrator-lane entries (FAFF-536) have no mandate issue** — skip the per-issue comment; the run summary + `/faff-wtf` §4 are their sanctioned surfaces. Do **not** park the build. The human creating the root later via `/faff-jot` (→ `initiated: interactive`) is the approval.
...
- **Containment is a precondition, not a replacement for appetite** — a `contained` verdict still goes through the existing appetite gate, which decides whether to file at this level. **outward-new-root is never filed at any appetite including `full`** (the hard floor — gateway → **Appetite for destruction**). The FAFF-536 `outward-self-intake` classification (step 5 above, default-off, sanctioned per ADR-0079) is the sole carve-out: a same-repo core-defect that earns it files to the intake bucket instead — it does not weaken this sentence, because everything that remains `outward-new-root` still hits the floor. **Trust boundary (FAFF-354):** `--ancestry` is agent-sourced, so the verdict binds structure, not truthfulness — `--record <run_id> --phase <phase>` binds it to the exact payload walked so `faff audit` can recompute-and-compare post-hoc, a **detective**, never preventive, control (never fetches the tracker itself).
...
- **At run start** (run-id minted, before step 4): write `owner` with `status:"running"`, `pid`, `session_id:<run-id>`, `started_at:now`, `last_heartbeat:now`, **and export the per-session pointer** so this session's own Stop hook recognises its own run:
  ...
  **Immediately after, spawn the detached watchdog poller** (FAFF-470, ADR-0065's primary invocation locus — the checkpoint consult above is cooperative; this lane covers a run that stops reaching checkpoints at all): `"$faff" sentry-poller start --run-dir "$FAFF_RUN_DIR"`. One line — all spawn/lifecycle mechanics live in the CLI, not here (deterministic-tools-over-prose). Spawned on **every** run regardless of level; it only **acts** (aborts) on an L4-minted ledger, mirroring _The interrupt_'s mint-scoped table.

  **Resolve the budget envelope and baseline tokens here too** (FAFF-36/FAFF-558/FAFF-552): resolve the `BudgetEnvelope` from the `budget:` config block + `--until`/`--max` flags into the ledger `budget` block before step 4, then snapshot the run-start baseline with the **deterministic write-once subcommand** — `"$faff" budget baseline --run-dir "$FAFF_RUN_DIR" --root "$measure_root" --session-id "$session_id"` — rather than hand-writing `tokens_at_start` from a `budget check` reading (that prose hand-write was the exact fragile step FAFF-552 traced the mid-run-compaction false-trip to). The subcommand persists the baseline **triple** — `tokens_at_start_by_model_class` (per-model), the derived scalar `tokens_at_start`, and `measure_session_id` (the run's owning measuring session, FAFF-552, so a later `check` prefers it over a drifted ambient session after a mid-run compaction) — in one call, is a no-op if the baseline is already set (write-once keyed on `measure_session_id`, so a re-run/resume never re-snapshots against an enlarged post-compaction transcript), and degrades safely (records the session with a zero baseline, `reason:"estimate-degraded"`) when no transcript is yet resolvable — never a crash, never a non-zero exit a caller could misread as a breach. **Also capture `budget.measure_root=$PWD`** (FAFF-488, before any worktree work) **and `session_id=$CLAUDE_CODE_SESSION_ID`** (already stamped into every `BuildDispatch.session_id`): pass both as `--root "$measure_root" --session-id "$session_id"` on every later token call (including this one), so an Agent-thread cwd reset never mis-resolves the transcript. **(FAFF-408) If you token-tag events**, also seed `budget.tokens_at_start_by_class = {input:0, output:0, cache_write:0, cache_read:0}` here (the four-class baseline the first `--tokens` event's checkpoint advances from); a stale/absent value degrades safely (the CLI falls back to zeros).
...
- **At orchestrator exit** (every path — clean drain, all-parked, budget-hit): set `owner.status:"done"`, **and in the same edit record `stop_reason: "<the summary's Stop reason token>"`** (FAFF-396 — the machine-readable form of the `## Reporting` `Stop reason:` line, so a headless `faff disposition` sees a run-level escalation after the container dies; informational, outside the completeness invariant). A run that exits with `status:"done"` but admitted-without-outcome work still trips `runcheck` (that's a real bug) — `done` only tells the hook "no live owner is holding this", it does not exempt the completeness invariant. **Alongside this close, stop the watchdog poller** (FAFF-470): `"$faff" sentry-poller stop --run-dir "$FAFF_RUN_DIR"`. Belt, not brace — the `owner.status:"done"` write above is the brace: the poller self-stops within one interval on it even if this line never runs.
...
## Unit economics — call `faff economics --run-dir <run-dir> --json --root "$measure_root" --session-id "$session_id"` (pure) after step-11 runcheck; the CLI computes, beep-boop only renders (FAFF-357)
