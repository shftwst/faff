...
5. **Wave-boundary stop.** **Default:** queue empty → exit to reporting; else run steps 5–7 again (conflict analysis → build pass → wave drain) and return to step 8 — termination by queue emptiness, no iteration cap (the long-standing default, unchanged). **Under `--converge`:** instead delegate to **`faff run-done`** (the FAFF-38 predicate — composes the signals into `run-complete` | `continue` | `escalate` over a fixed safety floor + the methodology's `run-termination-policy` ladder; gateway → **The `methodology` slot**). Resolve the binary (gateway → **Resolving the `faff` executable**) and call `faff run-done --json` with `RunSignals` as flags — `--queue-empty`/`--all-parked` (8.4 queue), `--ledger-clean` (`faff runcheck`-clean), `--budget '{breached,outcome}'` (8.1's), `--no-prd` (unless a PRD is in scope), `--non-convergence` (the backstop below), `--policy '<ladder>'` (the methodology's, else omit) — then branch on the JSON `verdict`: **`continue`** → re-enter (steps 5–7 → step 8), folding in chain-unlocks *and* the eligible discovered scope from 8.0; **`run-complete`** → both tributaries dry, exit with `Stop reason: converged/both-dry`; **`escalate`** → exit with the run-done `reason` (`non-convergence` / `budget-escalated(<dims>)` / `product-incomplete`) + the needs-human signal — **reported**, never a silent truncation. **Non-convergence backstop:** keep a `no_progress_waves` ledger counter — reset to 0 when the wave made progress on **either** bottom-up tributary (`filed_this_wave > 0` **or** it built ≥1 newly-admitted issue, e.g. a chain-unlock), increment only when **both** were dry; at **K = `convergence.max_waves`** (default 6) pass `--non-convergence` above. Resetting on either tributary is deliberate: a run still productively draining a deep chain (filing no *new* discovered items) is **not** stalled and must not false-escalate — the backstop catches only a loop re-entering with genuinely **no** progress on either axis. The cap never decides the *normal* exit (dryness + budget do), and firing it is always reported.

Log each wave's admissions, drain count, `filed_this_wave`, `no_progress_waves`, the `run-done` verdict (under `--converge`), and exit reason under `.faff/runs/<run-id>/wave-N/`. Exit reasons are `drained` (queue empty after build pass), `all-parked` (every remaining issue parked or errored), `budget-hit` (budget gate fired during or at the boundary of this wave), or — under `--converge` — `converged/both-dry` and `non-convergence` (the run-done verdicts above).
...
- (**`--converge` only**) `faff run-done` returns a terminal verdict at a wave boundary (step 8.5): `run-complete` → `Stop reason: converged/both-dry` (both bottom-up tributaries dry); `escalate` → `Stop reason: non-convergence` (the runaway backstop fired — a reported bug signal, never a silent truncation) or `budget-escalated(<dims>)` / `product-incomplete`, with the structured needs-human signal emitted. Without `--converge`, termination is by queue emptiness exactly as before and run-done is not consulted.
...
- **Always wave-loops until queue stability.** The build phase is wave-based — after each wave's queue drains, the orchestrator re-assembles to pick up work unlocked by issues that just shipped. By default the run ends only when a wave assembles to an empty build queue: termination is by emptiness, with no iteration cap. Under **`--converge`** the loop additionally folds in freshly-filed execution-discovered scope and delegates the wave-boundary stop to `faff run-done` (genuine dryness + budget + a non-convergence backstop) — still no fixed iteration count as the primary mechanism; the hard cap (`convergence.max_waves`) only ever fires as a *reported* runaway backstop.
