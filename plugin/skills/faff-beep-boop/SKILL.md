...
7. **Call `faff run-start` and branch on the verdict.** `faff run-start --signals '{"target_resolved":<t>,"outward":<o>,"prd_present":<p>,"prd_ambiguous":<a>,"prd_admissible":<ad>,"coverage_measurable":<cm>,"coverage_covered":<cc>}'`. A malformed `--signals` (exit 2) is treated as `refuse` (fail-safe), surfaced as a diagnostic reason. Branch on `verdict.verdict`:
   - **`plan`** (`coverage-thin`) — invoke `/faff-plot --autonomous` against the resolved target (it **self-mints** its own L4 ledger at ignition — see `faff-plot/SKILL.md` → Ignition). Mint **nothing** here and do **not** proceed to step 1 (tidy) — **decompose-only**: this run does not also drain. STOP.
...
5. **Wave-boundary stop.** How a wave's empty (or all-parked) build queue terminates the run depends on the run's **level**, whether the run is **opted out** of convergence (`--no-converge`, or `convergence.enabled: false` with no `--converge`), and (FAFF-559) whether the run is git-only. **`--converge` and `--no-converge` are mutually exclusive — passing both is a refused usage error, announced at run start, never silently resolved to one;** a flag overrides `convergence.enabled`, and at L4 both flag and knob are **inert** (the loop is forced on):
   - **A lights-out-minted (L4) run — convergence is non-optional:** doneness terminates, never bare queue-emptiness. Consult **`faff run-done`** at run-end and let its verdict decide `run-complete` vs `escalate` vs `continue`, **and run the within-run convergence loop regardless of the flag/knob** — the step-8.0 discovered-scope fold and the non-convergence backstop apply, and `--no-converge` / `convergence.enabled: false` are inert. This is what makes run-done the L4 **terminator**: an L4 run that empties its queue while its budget escalated or its PRD is unsatisfied **escalates to needs-human** rather than silently reporting "complete".
...
**`holdout_step(spec, prdr_id, key, run_dir) → meets-spec | gaps | fails | needs-human`** — call-site-agnostic, so the per-issue graft merge-gate reuses it unchanged with a per-issue association. **`spec` is contractually the full view** (`faff dod split --view full` of the tracker spec; git-only: `.faff/specs/<issue>.md` — never a committed builder-view doc), and both callers — this per-run phase and graft Step 10's per-issue holdout gate — resolve and pass exactly that (byte-identical to the raw spec when marker-free, so the contract is a no-op there):
