...
- **`outcomes`** — written the moment an issue reaches a terminal bucket: `shipped`, `pr-open`, `parked`, `errored`, `routed-out` (routed out by the verdict gate at step 4), `unreached-budget` (admitted but a budget flag fired before dispatch), or `superseded` (FAFF-571 — the issue reached Done because its deliverables were already merged to `main` by *other* tickets; this run built nothing and opened no PR for it). These are exactly the run-summary buckets — the ledger is the structured twin of the summary.
