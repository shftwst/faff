...
| `cost` | — | `budget.cost` | Dollar cost | **The recommended default spend governor.** Prices this run's per-model × per-class token spend from a built-in price map by default (no `price_per_mtok` needed — a model the map doesn't know about prices at the map's costliest known rate rather than ever silently undercounting). An explicit `budget.price_per_mtok > 0` keeps the legacy flat `cost = tokens × price_per_mtok` scalar instead (deprecated but honoured verbatim, human-explicit outranks the default). (Real CI/compute metering is out of scope — `cost` is priced from token spend until a CI-introspection producer lands.) |
...
budget:
  ...
  cost:           # dollar ceiling — priced per-model x per-class from the built-in map by default
  at_ceiling: stop          # stop | narrow | escalate  (L3 default stop; a lights-out run mints escalate when unset)
  price_per_mtok: 0         # DEPRECATED — set only to keep the legacy flat-scalar cost byte-for-byte;
                            # unset (default) prices `cost` from the map instead
