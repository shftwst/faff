...
- **At orchestrator exit** (every path — clean drain, all-parked, budget-hit): set `owner.status:"done"`, **and in the same edit record `stop_reason: "<the summary's Stop reason token>"`** (FAFF-396 — the machine-readable form of the `## Reporting` `Stop reason:` line, so a headless `faff disposition` sees a run-level escalation after the container dies; informational, outside the completeness invariant). A run that exits with `status:"done"` but admitted-without-outcome work still trips `runcheck` (that's a real bug) — `done` only tells the hook "no live owner is holding this", it does not exempt the completeness invariant.
