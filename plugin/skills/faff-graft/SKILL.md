...
Invoke the `review` slot via the Skill tool (resolve per gateway → **Sibling-skill invocation**; the default `faffter-noon-review` is a canonical name), passing the diff (`git diff main...HEAD`), the spec, the test results, and the Step 8 AC checklist. **Also forward the resolved lights-out signal** — the same `$lights_out` boolean (`level:"L4"`) graft already resolved once for the admissibility gate (Step 2) and the holdout gate (Step 10), defaulting `false` when it is unresolved. This is the single L4-resolution locus fed to a third consumer, not a second independent resolution: a review occupant that gates on the lights-out path (an adversarial reviewer escalating a `critical` to `needs-human`) reads it from here. Forward it as prose context in the Skill-tool invocation (alongside the diff/spec/Phase-1 result), not a CLI flag. The slot's default is `faffter-noon-review`; the review's passes and how it arrives at a verdict are that skill's concern, not faff-graft's. faff-graft owns only the sequencing around the result.
