...
5. `git add -A docs/adr/ && git commit -m "docs(adr): renumber <ISSUE-XX> ADR <old>→<new> (merge-time collision)"`, push, then proceed into the CI re-confirm on the renumbered head. `faff merge-gate` observes CI on the **new head sha** — if CI doesn't re-run there it returns `no-ci-coverage`/`not-ready` (fail-closed; surface, never a silent merge). The guard is idempotent and convergent under the merge-lock — a racing third merge onto the same `next` re-fires on the next rebase.
