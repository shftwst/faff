...
3b. **Detect contradictions + offer/admit supersession (L3 offer — FAFF-198; L3 autonomous admit-gate — FAFF-199; runs unless `adr.mode: off`).** With the new `## Decision` body in hand and its number known (both pre-commit), check whether it contradicts a **live** (non-superseded) ADR, and link them via the shipped `faff adr supersede` — interactively on human confirm, or autonomously under the deterministic `faff adr admit` two-gate below (never a free flip either way):
   ...
   - **Route each `contradicts:true`** (the offer-routing table is `faff adr`'s — mechanical, never prose-improvised):
     ...
     - **Autonomous (any `adr.mode` ≠ off) — admit-gated auto-supersede (FAFF-199).** The hard floor carves out exactly **one** case: a **loop-provenance** old ADR may auto-supersede when `faff adr admit` returns `admit` (gateway → **Appetite for destruction** → hard floor). Every other case (human/legacy-provenance old ADR, or any gate refusal) stays refused-autonomous, unchanged from FAFF-198. Procedure: (1) `provenance := <adr>.provenance` from the candidate set above (no second round trip). (2) Run the **drift challenge** — invoke the `review` slot (`faff config get slots.review`; a different model, the FAFF-256 Phase-2 pattern) with `{old Decision, new Decision, why}` → `survived`/`overturned`; unreachable after fallback → **absent** (omit `--challenge` — a missing skeptic is a reject, never a pass). (3) `lineage := in-window supersessions on <old>'s lineage` (walk `Supersedes:` + `Date:` via the shipped ref-parsers, window = `adr.thrash_window` days). (4) `verdict := faff adr admit --actor loop --supersedes-provenance <provenance> [--self if <old> governs this run's own increment] [--challenge <outcome> if it ran] --lineage-supersessions <lineage>` → pipe to `faff contract adr-admission` (fail-loud on malformed, never coerced toward `admit`). (5) **`admit`** → `faff adr supersede <old> --by <new-id>` as part of the materialisation commit (sub-step 4), log the verdict + challenge outcome to the run record. (6) **`propose-only`/`reject`** → today's FAFF-198 behaviour: write/overwrite `.faff/adr-proposals/<new-id>.json` (`{new-id, adr, why, disposition, reasons}` — the artifact `/faff-wtf` §4d reads) and log; no `docs/adr/` write; proceed. Appetite grades only a `reject`'s digest prominence; a `propose-only` always persists (it's what the ratification gesture surfaces). (7) Any step error → log, fall back to (6), never block the build.
...
`needs-human` is reserved for things the merge-confidence gate can't catch. The revert test (if `git revert` on the merge commit fully undoes the change, it is not `needs-human` — it is `pass` or `fail`) is part of the fixed review-verdict contract in the gateway. `unavailable` is a distinct availability-axis signal (the review never ran at all), never a verdict itself — it is never `pass`. See the gateway's Autonomous Mode Contract for the full rule on what escalates vs. what proceeds.
...
4. Run Step 9 (review phase) — **pre-PR, no PR open yet** (review iterates against the branch/diff; findings → the **tracker issue**, never a PR). The terminal-verdict comment is posted-or-updated by the comment-identity contract — locate by its marker pair, create/update/reconcile per **gateway → Review-findings comment identity** (FAFF-202), the same in autonomous mode. Act on the four-valued signal:
   ...
   - `fail` → iterate: fix flagged items, re-run tests, re-run review — **still pre-PR, no PR opened**. Loop until `pass` or `needs-human`, escalating to `needs-human` once iterations reach the **`review` slot's appetite-scaled iteration cap** (resolved, never stated here: `appetite := faff config get appetite`; `cap := faff review-iteration-cap --appetite <appetite>`; FAFF-341) — this file carries no per-appetite integer of its own.
...
6. Run Step 10 (merge-confidence gate) automatically (**git-only mode, FAFF-526:** no CI to wait for — per Step 10's Git-only mode note, skip the CI-wait/classify sub-steps and hand off directly to the `ship` producer with `--local`, never `--pr`; map its outcome identically):
   - **All applicable conditions hold:** wait for CI to reach a terminal state (`gh pr checks --watch`), classify the result per _Classifying the CI result_; on `ci-green`, **under the L4 lights-out signal run the _Holdout gate_ (condition 4) last** — a holdout-block flips the PR to draft and returns `pr-open-for-human` (bucket `pr-open`) without merging; only on a `meets-spec` pass (or at L1–L3, where the gate does not apply), run the **_ADR-accept sub-step_ (FAFF-546, above)**, then hand off to the `ship` producer (resolve `faff config get slots.ship`) and pipe its `faff-contract:delivery-outcome` block to `faff contract delivery-outcome`, then map the outcome to a caller-facing return: `shipped` → **run _Post-merge verification_ (below) first, then** `shipped` (worktree eligible for cleanup, chained issues unblock); `not-ready:<reason>` → park retry-later, return `pr-open-for-human`; `failed:<reason>` (including an unmappable result coerced to `failed`) → one fix attempt if obvious from the error, else park, return `pr-open-for-human`.
