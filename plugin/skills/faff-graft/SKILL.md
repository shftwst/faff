...
**Capture the graft-side core-loop decisions (dispatched/autonomous only, best-effort, flag-guarded).** When a resolvable run dir is present — the dispatched/autonomous path, where the orchestrator pre-set `$FAFF_RUN_DIR` with a genesis `events.jsonl` chain (standalone-interactive graft mints its substrate only *after* these Step-2 gates, so it captures nothing here — never a call that silently drops) — record this build's `next` and `eligible` decisions into the coordination-fidelity corpus. Guarded on `capture.decision_kernel` (`faff decision-capture record` no-ops unless it reads exactly `on`) and best-effort (a missing run dir / malformed input logs a degraded note and exits 0 — never branch on it); `--run` = the `$FAFF_RUN_DIR` basename, `--issue` = the issue being built, piping `{normalised_inputs, selected_action}` on stdin:
- `--kernel next` (the Step-2 prep gate) — `normalised_inputs` = all seven `nextStep` keys (incl. `awaitingSpecReview`, so the record stays in the study's `matrix.next`, not its `input-uncaptured` exclusion); `selected_action` = `graft` (the proceed path). A dispatched/autonomous graft only ever reaches this capture on a pre-vetted, spec-present issue, so a build that proceeded here despite a non-`graft` verdict is exactly the wasteful divergence the study catches — the `prep` bail never reaches this point.
- `--kernel eligible` (this eligibility gate) — `normalised_inputs` = `{labels, automationDefault, trackerPresent}` as fed to `faff eligible`; `selected_action` = `eligible` when graft proceeded past this gate, `ineligible` when it refused and returned the ineligible skip.
Record the action actually taken, never a re-emitted verdict — that prescribed-versus-actual gap is the signal the fidelity study measures.
