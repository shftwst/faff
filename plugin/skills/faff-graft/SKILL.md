...
- **Short-circuit (no env when nothing is born-verifiable).** First run **`faff dod classify --spec <spec>`**; if it reports **zero** born-verifiable criteria (scenario/assertion) the evaluator could only return all-`needs-human`, so provisioning buys nothing — **skip provisioning** and treat the gate as a **holdout-block** with reason `no born-verifiable criteria` (route as below). Otherwise provision **at most one** env for this issue.
