...
**Foreground posture (FAFF-491 — governs Steps 7–8b, both modes, non-negotiable).** Every build/gate/test command — the gate ladder above, any per-AC test at Step 8, any lint/type/unit invocation the build issues — runs as a **foreground** Bash call. Never issue it with `run_in_background: true`, and never end a turn with a background job still in flight: a turn that ends this way does **not** resume when the job finishes — the subagent simply stops, and nothing restarts it until heartbeat staleness (900s) flags the run for a manual ground-truth recovery (this mirrors Step 11's "there is no background" CI-wait rule, applied here to the build phase itself, where two live stalls actually happened — FAFF-466, FAFF-446). A run that risks exceeding the Bash tool's max timeout is **chunked or polled across successive foreground calls**, never backgrounded — the FAFF-234 heartbeat ticks already bracket the wait, and the 900s staleness window comfortably exceeds any single blocking call. This posture is checked at two further layers so it survives prose drift: `faff validate-adapters` FAILs this file if either anchor phrase above is missing (Layer 2), and — where the harness's PreToolUse hook event actually exposes `run_in_background` — the registered `background-fence` hook denies a backgrounded gate-family Bash call outright (Layer 3).
