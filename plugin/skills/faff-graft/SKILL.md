...
- `--kernel next` (the Step-2 prep gate) — `normalised_inputs` = all seven `nextStep` keys (incl. `awaitingSpecReview`, so the record stays in the study's `matrix.next`, not its `input-uncaptured` exclusion); `selected_action` = `graft` when the spec was present and the build proceeded, `prep` when it bailed for a missing spec.
