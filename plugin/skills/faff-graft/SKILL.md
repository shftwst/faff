...
- **`lights_out`** — the same `$lights_out` boolean (`level:"L4"`) graft already resolved once for the admissibility gate (Step 2) and the holdout gate (Step 10), defaulting `false` when unresolved. This is the single L4-resolution locus fed to a third consumer, not a second independent resolution. It is orthogonal to `autonomous` (L4 = `autonomous ∧ lights_out`); forward it alongside for L4-specific review behaviour and future composition.
