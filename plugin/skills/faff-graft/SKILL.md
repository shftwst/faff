...
- **`autonomous`** — `true` for **any** autonomous dispatch (every beep-boop build, L3 or L4), `false` on an interactive graft. This is the signal a review occupant keys unattended-run behaviour off: an adversarial reviewer escalates a Phase-2 `critical` to `needs-human` and loudly annotates a full-chain outage on `autonomous:true` (a soft `critical` or a silent skip must not slip through when no human is watching, which is exactly the overnight L3 case). Interactive graft forwards `false`, so those paths stay advisory.
