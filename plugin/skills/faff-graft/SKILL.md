...
**Resume-at-review check (FAFF-402 — autonomous L3/L4 only; interactive skips this, like the FAFF-329 review checkpoint).** Before creating a fresh worktree, read the build-complete checkpoint — `faff build-progress read "$run_dir" <ISSUE>`. On **exit 3** (no checkpoint) see the **resume-store fallback** immediately below before creating a fresh worktree. On **exit 0**, `git fetch origin` and recompute `cur = sha256(git diff origin/main...origin/<branch>)`:
...
Opens no PR and never merges — `faff merge-gate` does **not** read build-progress (a resume artifact, not a merge-floor assertion). **Git-only mode (FAFF-526, no pushable remote, detected/never forced):** Step 1 no-ops (nothing to push to); `diff_hash` is instead `sha256(git diff <base>...<branch>)`, the local three-dot diff against the locally-resolved base (`merge-gate --local`'s own main/master resolution); Step 3 is unchanged (the branch ref survives worktree teardown, so resume re-adds a worktree with no push involved).
...
Invoke the `review` slot via the Skill tool (resolve `faff config get slots.review` per gateway → **Sibling-skill invocation**; a bundled default is a canonical name), passing the diff (`git diff main...HEAD`), the spec, the test results, and the Step 8 AC checklist. **Also forward two orthogonal signals — `autonomous` and `lights_out`:**
