...
- Else transition it `→ In Progress`. That is the claim. **Then apply the `faff-claimed` breadcrumb** — `faff label add <issue> faff-claimed` and its descriptor's write (gateway → **Control-label provisioning**), one write at the **same cadence** as the status write. Its presence is the provenance that lets `/faff-tidy` later auto-reclaim this claim if it goes stale (past `claim_ttl_hours`); a claim without it is treated as human-set and only surfaced, never reverted. (git-only mode: the `faff label` op is a no-op — no tracker to write to, and no cross-host stale-claim path exists there.)
