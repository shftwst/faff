...
| `unavailable` | No review verdict could be produced — a mandatory review-chain outage (provider down), not a verdict (FAFF-405). Dispositioned identically to `needs-human` today (retry-later reclassification is FAFF-403). | Flip PR to draft. Park per the shared park protocol. Do not auto-merge. |

`needs-human` is reserved for things the merge-confidence gate can't catch. The revert test (if `git revert` on the merge commit fully undoes the change, it is not `needs-human` — it is `pass` or `fail`) is part of the fixed review-verdict contract in the gateway. `unavailable` is a distinct availability-axis signal (the review never ran at all), never a verdict itself — it is never `pass` and always parks, same as `needs-human`. See the gateway's Autonomous Mode Contract for the full rule on what escalates vs. what proceeds.
...
- **`unavailable`** — no review verdict could be produced (a mandatory review-chain outage, FAFF-405) — → surface on the tracker issue and park per the shared protocol **without opening a PR**, EXACTLY as `needs-human` above (same no-PR human handoff; FAFF-403 will later reclassify this to a retry-later disposition, not this ticket).
...
4. Run Step 9 (review phase) — **pre-PR, no PR open yet** (review iterates against the branch/diff; findings → the **tracker issue**, never a PR). The terminal-verdict comment is posted-or-updated by the comment-identity contract — locate by its marker pair, create/update/reconcile per **gateway → Review-findings comment identity** (FAFF-202), the same in autonomous mode. Act on the four-valued signal:
   ...
   - `unavailable` → no review verdict could be produced (mandatory chain outage, FAFF-405) — surface on the tracker issue and park per the shared protocol **without opening a PR**, EXACTLY as `needs-human` above. Return `needs-human` (same no-PR handoff and ledger bucket; FAFF-403 will later give `unavailable` its own retry-later return, not this ticket).
...
- `needs-human` — a **pre-PR, no-PR** human handoff: **review** returned `needs-human` **or `unavailable`** before any PR was opened (Step 9, pre-9b) — a review-chain outage (FAFF-405) dispositions identically to `needs-human`, no separate return token (no downstream behaviour change; FAFF-403 later gives `unavailable` its own retry-later return). Surfaced on the tracker issue and parked per the shared protocol; **no PR exists**. Distinct from `pr-open-for-human` (which has a PR); maps to the `parked` ledger bucket (see below).
...
**Ledger bucket mapping.** These caller-facing returns map onto the run-ledger terminal buckets the `concurrency` slot records: `shipped`→`shipped`, **`pr-open-for-human`→`pr-open`**, **`needs-human`→`parked`** (the no-PR review handoff parks — there is no open PR to track; a review-verdict `unavailable` — chain outage, FAFF-405 — folds into this same `needs-human`→`parked` mapping, not a new bucket), `parked`→`parked`, `errored`→`errored`. The slot writes the ledger *bucket*, not the raw return token, or `runcheck` flags an invalid outcome.
