...
_**Interactive custody stamp sub-step (L4 `--local`, top-level graft only).**_ On the top-level interactive (non-dispatched) `--local` path **at L4 only** (skip under a dispatch cut, at L1–L3, and on `--pr` — nothing consumes it there): after Steps 8/9, produce a per-issue custody verdict over the seven-entry forge surface by composing two shipped primitives with no code change — `integrity-digest snapshot --run-dir "$run_dir" --issue <ISSUE-XX>` piped to `integrity-digest verify --run-dir "$run_dir" --manifest - --issue-context <ISSUE-XX> --merge-state pre-merge --record-result "$run_dir/<ISSUE-XX>/custody-verdict.json" --json` — then thread `--custody-verdict <path> --custody-verdict-sha256 <result.verdict_sha256>` (the persisted bytes’ hash, never a re-hash) to the `ship` producer. Thread them on any non-`clean`/absent outcome too: `computeCustodyVerdictAdmission` refuses a non-clean or absent verdict, so the floor refuses (uncertainty fails toward refuse). **This is a self-consistency stamp, NOT the dispatched detective-custody boundary** — the trusted session attests its own surface, it does not detect an untrusted lane — so it is **never** fed through `FAFF_INTEGRITY_BOUNDARY` and never labelled custody. The merge-floor’s digest-basis admission is a separate change; until it lands the producer half writes and threads the verdict but `merge-gate --local` does not yet key on it.
