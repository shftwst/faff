...
_**Interactive custody stamp sub-step (L4, top-level graft only — `--local` or `--pr`).**_ On the top-level interactive (non-dispatched) merge path **at L4 only** (skip under a dispatch cut, and at L1–L3): after Steps 8/9, produce a per-issue custody verdict over the seven-entry forge surface by composing two shipped primitives with no code change — `integrity-digest snapshot --run-dir "$run_dir" --issue <ISSUE-XX>` piped to `integrity-digest verify --run-dir "$run_dir" --manifest - --issue-context <ISSUE-XX> --merge-state pre-merge --record-result "$run_dir/<ISSUE-XX>/custody-verdict.json" --json` — then thread `--custody-verdict <path> --custody-verdict-sha256 <result.verdict_sha256>` (the persisted bytes’ hash, never a re-hash) to the `ship` producer. **Production is mode-agnostic — there is no `--local`/`--pr` gate here**: the step snapshots the same run-dir forge surface regardless of how the merge will land, and the `ship` producer (below) forwards the two flags onto whichever `merge-gate` invocation it makes. Thread them on any non-`clean`/absent outcome too: `computeCustodyVerdictAdmission` refuses a non-clean or absent verdict, so the floor refuses (uncertainty fails toward refuse). **This is a self-consistency stamp, NOT the dispatched detective-custody boundary** — the trusted session attests its own surface, it does not detect an untrusted lane — so it is **never** fed through `FAFF_INTEGRITY_BOUNDARY` and never labelled custody. FAFF-892's merge-floor digest-basis admission is landed and path-agnostic (`cmdMergeGate` and `cmdMergeGateLocal` share the same `buildMergeFloorDigestVerify` + `resolveIntegrity` fold), so threading these flags on the `--pr` invocation admits identically to `--local`.
