...
**Deterministic accept-bar (arithmetic over the bundle, not the judge's read).** Coerce a judge `accept` to `park-needs-human` whenever `evidence.blocker_free_latest == false` (a standing blocker is never shipped by weighing) **or** `evidence.infosec_major_free_latest == false` (the security-severity floor — a standing `infosec` objection at `major` or above is never down-weighted to taste). Then apply the **authority gate**, a pure function of the coerced verdict × level:

| verdict | L4 | L3 |
|---|---|---|
| `accept` | promote-final — retain `spec-review: accept (judge, L4-final)` | promote-provisional — retain `spec-review: accept (judge, L3-provisional)`; log the evidence bundle + rationale; a later human comment classified as a challenge re-parks/reverts via **Live-thread reconciliation** |
| `keep-going` | grant the next round, re-apply this gate at the next would-be-park (bounded below) | same, bounded |
| `park-needs-human` | park (`faff-parked`) | park (`faff-parked`) |
