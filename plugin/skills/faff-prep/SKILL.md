...
**Ratified-scope assembly (loop entry, once — the design lenses' deferral input).** Also at loop entry, before the first review round, assemble the `## Ratified scope` block once so every lens receives the same bytes across the loop's rounds (the block is stable across a spec's review rounds — assemble it once, never per round). Resolve the container from the L4 run ledger's `prd_root_container` field (written by `lights-out.js`); when a container resolves, run `faff ratified-scope --assemble` and, on exit 0, write the block to `$scratch/ratified-scope.md` — the file the occupant appends to all four lenses' `--context`:
...
if [ -n "$container" ] && "$faff" ratified-scope --assemble --container "$container" > "$scratch/ratified-scope.md" 2>/dev/null; then
  : # exit 0 — block written; the occupant delivers it to every lens as --context
else
  rm -f "$scratch/ratified-scope.md"  # no container, or exit 3 (nothing ratified) / exit 2 (source unreadable): no deferral this loop
fi
```

No `prd_root_container` (no run dir, an L1–L3 run, or an unresolvable container), or CLI exit 3/2 → **no** `$scratch/ratified-scope.md`, and every lens behaves exactly as today (no deferral). The CLI fails loud on its own channel on an unreadable source (exit 2); prep never synthesises a block.
