...
slots:
  ...
  prd_validator: faffter-noon-prd-validator          # PRD-admissibility validator for the L4 run-start gate, optional (default faffter-noon-prd-validator)
...
| `prd_validator` | `faffter-noon-prd-validator` | Code-blind PRD-admissibility **validator** for the L4 run-start gate: given only a PRD document (never the codebase/tracker), it judges whether the PRD's stop-conditions are machine-verifiable and how wide the implementation's creative licence is, and emits a `faff-contract:prd-readiness` block the run-start pre-step pipes to `faff contract prd-readiness` (`admissible` → admit the run; anything else → refuse, fail-safe). A producer doing-skill. |
