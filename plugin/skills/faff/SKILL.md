...
3. **Record every terminal outcome to the run ledger** the moment an issue lands, as one of the fixed buckets `shipped` / `pr-open` / `parked` / `errored` (see **`.faff/` logging directory** → Run ledger). Map `/faff-graft`'s caller-facing returns to buckets: **`pr-open-for-human` → `pr-open`**, **`retry-later` → `parked`** (FAFF-403 — additionally append the issue id to the ledger's top-level `review_outage_pending` array, mirroring `review_adversarial_skipped`; no new bucket), others as themselves. Write the bucket, never the raw token, or `runcheck` flags it invalid.
