...
- **L3 (on the loop)** — the call-site (`/faff-plot` at project creation, `/faff-jot`, or on-demand) **surfaces the `Proposed` PRDR for human approval**; the human ratifies by flipping `Status: Accepted` in the tracker (FAFF-255's human gesture). The machine never self-accepts.
- **L4 (lights-out)** — the runner routes the `Proposed` PRDR through **`faff prdr admit`** (FAFF-255) carrying the FAFF-256 (upper-YAGNI) + FAFF-257 (lower-coverage) verdicts; on `admit` it self-Accepts **within 255's two gates + FAFF-222 containment + the appetite floor**; on `propose-only`/`reject` it escalates (needs-human) / parks. It **never** auto-self-defines a vacuous DoD.
