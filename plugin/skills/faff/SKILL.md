...
| **L2** | spec-before-build (the `prepcheck` Stop-hook blocks session-end on a produced-but-unattached spec); worktree isolation (`setup-worktree.sh` + the `faff worktree-root` resolver); every build ships as a reviewable PR (revert-via-PR flow) | that each gate is actually *presented* before the `Skill` tool fires — a "prose contract … not statically lintable" (the standalone-gate limit); the review-verdict + AC judgement |
