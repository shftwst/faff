...
| faff-graft | Skip prompts. Mid-build ambiguity → invoke the `faff-prep` skill respec. Still ambiguous → park. Post-build → AC verification → review (pass/fail/needs-human/unavailable). `pass` → auto-merge on green CI (unblocks chained issues). `fail` → iterate. `needs-human`/`unavailable` → flip PR to draft, park (identically). |
