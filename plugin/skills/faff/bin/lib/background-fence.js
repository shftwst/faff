...
// variable ladder invocation (`"$faff" gates run`). The `gates run` pair is
// faff's own subcommand shape (low collision risk with any other tool), so it
// is matched without requiring a literal `faff` token — a deliberate, narrower-
// than-merge-fence trade documented below and pinned by an explicit selftest
// deny case.
//
...
const GATE_FAMILY_PATTERNS = [
  // faff gates run — the Step 7.5 ladder invocation. "gates run" is matched as
  // an adjacent token pair without requiring the literal `faff` binary token,
  // because the ladder's own shape quotes+variables it (`"$faff" gates run`),
  // which would otherwise break a `faff`-anchored match.
  /\bgates\s+run\b/,
  // node --test
  /(^|[^\w])node\b[^\n]*(^|\s)--test(\s|=|$)/,
