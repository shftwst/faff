...
const { dig, findRoot, isSafeAnchorRelPath } = require("./shared-infra");
const { loadConfig, DEFAULTS } = require("./config");
const { diffAgainstManifest, sha256 } = require("./integrity-digest");
const { verifyChain, verifyEffectsChain, appendEventRecord } = require("./events");
const { computeBundleVerdict } = require("./contract-defs");
// FAFF-1000 — the denylist-clean sealing core (buildBundle, readAnchorDir, the local BundleStore
// occupant, identity validation, canonicalJSON, the b1/b2 required-member gate + its constants)
// lives in bundle-seal-core.js so the standalone commissaire facade can require it WITHOUT reaching
// this file's config.js / contract-defs.js graph. Imported here rather than kept as a second copy —
// bundle.js's own "never forked" convention; every symbol re-exported below resolves to that one
// implementation. The git-remote occupant, resolveBundleStore*, classifyBundle, publishBundle and
// the claim stores stay here (they need config.js / contract-defs.js / node:child_process, none of
// which the seal core may touch).
const {
  BUNDLE_MANIFEST_VERSION, BUNDLE_BOUNDARY_KINDS, REQUIRED_MEMBERS, REQUIRED_MEMBERS_B1, REQUIRED_MEMBERS_B2,
  requiredMembersFor, canonicalJSON, validateIdentityForHandle, buildBundle, localBundleStore, localBundleDir,
} = require("./bundle-seal-core");
