...
const SKILL_LINE_BASELINE = { faff: 80, "faff-beep-boop": 773, "faff-graft": 950, "faff-prep": 625 };   // FAFF-1120 +2 faff-graft (the Governed tracker-write record rule: brackets every agent-mediated status save_issue with a tracker-write declare/observe on a governed run, mirroring the label-write rule; the run-start admit --scope now carries tracker-write in both faff-graft and faff-beep-boop — record-only, DETECTED not prevented; the status-set anti-pattern is held, no faff status set CLI). FAFF-1119 +8 faff-graft (four governed record-brackets: Step 8b push, Step 9b anchor-head push, Step 4b/4c ADR/decisions file-write, and the Step-5 governed label-write note that binds every control-label MCP write; all gated on governor.json — record-only, DETECTED not prevented). FAFF-1118 +6 faff-graft (Step 9b Governed pr-create producer half — declare/authorize/faff pr-create/observe — plus the faff pr-create wiring sentence in item 6 and the branch-delete authorize added to the Governed merge producer half; the run-start admit --scope now carries pr-create). FAFF-1115 +26 faff-graft (split the code-blind holdout into an OFFER and a RUN: Step 6 gains the decision-only `holdout_offer` sub-block writing `holdout-offer.json`; a NEW Step 8c runs the shared `holdout_step` after the Step-8b checkpoint (unattended keys on `verif.unattended`, never `legs.holdout`; attended keys on the marker); Step 10's Holdout gate reconciled from run→consume — no `holdout_step`, no first-time prompt, condition-4 + the merge-decision "L4-only run" reworded to "consume the Step-8c verdict at any level via the FAFF-1040 capability floor"; Step 3/9 resume-store now carries the Step-8c holdout.json for the idempotency skip. Net-lean after compressing both procedures to comment-annotated pseudocode). FAFF-1114 +1 faff-graft (Step 3 exit-2 switch gains a `clobbered-recoverable` case: shell `worktree-heal` to repair a worktree whose admin metadata was pruned out-of-band, then re-enter the staleness gate; exit-1 repair failure folds into the existing non-`no-worktree` park/WARN branch). FAFF-1034 +8 faff-graft (Step 3 run-start Commissaire admit bullet + Step 10 _Governed merge producer half_ subsection: resolve inputs / declare / authorize --level / block-on-deny / observe / conclude+reconcile, plus the inline schema:2-declare-skipped-on-governed note) +1 net beep-boop (run-start admit bullet + run-close per-issue conclude/reconcile bullet — promotes faff's own runner onto the Commissaire producer facade for the merge effect, fail-closed at the chokepoint). FAFF-1097 -2 faff-graft (Step 2 autonomous eligibility gate leaned to the single-signal model: dropped the `config get automation_default` read + the `--default`/`--tracker` args and the hold>automate>default precedence prose — eligibility is now `faff-automate` present ⇒ eligible, absent ⇒ not; baseline lowered to lock the reduction). FAFF-1052 +8 faff-prep (Reviewer pin paragraph: per-round window governance now runs via `spec-review-window --govern` instead of a hand-derived served-header-vs-pin comparison, plus the assert-the-governance-step-ran governance-<n>.json re-read + govern-record-missing fail-safe). FAFF-1068 +2 net faff-graft (Step 3 mine-once-at-run-start paragraph: `conventions mine` writes the schema-2 `.faff/conventions.json` at the start of Step 3 so the branch_naming/commit_subject/pr_title reads reuse one derivation on a source_fingerprint match — the cache-persistence half of ADR-0128 that FAFF-1041 designed but left unbuilt). FAFF-996 +23 faff-graft (Step 9: the build-review dialogue loop + would-be-park judge subsections, the `built-but-not-admitted` hold procedure, the generalised resume-store fallback branching on `cause`, the Return-values/Ledger-bucket-mapping entries — a bounded reviewer-author round-trip plus a blinded judge ported one altitude down from faff-prep's Spec-review gate). FAFF-1028 +1 beep-boop (checkpoint step 1a: `faff effects reconcile-merges` run immediately after `faff effects check`, combining `forbidden := check.any_escape OR reconcile.any_escape OR (reconcile.reconciled === false)` into the existing step-2 `--forbidden-side-effect` flag — the off-ledger landed-merge detection backstop). FAFF-966 +2 faff-graft (Step 9b anchor-refusal paragraph: a `no-events`/`genesis-invalid` anchor exit routes to the shared park protocol, pre-PR, never a repair — the sibling mint-seam-fix commit carries its own +2 beep-boop bump). FAFF-1041 +2 net faff-graft (Step 3 branch-name resolution paragraph via `faff conventions get branch_naming`; Step 4/4c spec+decisions commit-message grammar and the merge-time ADR renumber commit resolved via `faff conventions get commit_subject` in place of a hardcoded string; Step 9b PR-title resolved via `faff conventions get pr_title`, explicit `--title` only on divergence from commit_subject — line-neutral-ish net despite five touched sites since most replace an existing sentence in place). FAFF-1021 +8 faff-prep (Scenario A gains Step 1c: runs the shared already-shipped scan + premise-superseded gate before spec production, substituting explore findings + issue for the candidate spec, with a human scrap/narrow/proceed disposition in place of autonomous auto-park; the shared subroutine's intro paragraph now names all three call sites and their caller-supplied surface source / disposition). FAFF-1009 +4 beep-boop (§0a preflight capture note wiring run-start/run-outward under FAFF_RUN_DIR, and the wave-boundary capture note wiring run-done Tier-1 + queue-state Tier-2 mint-and-silence; next/eligible driver tokens folded inline, line-neutral). FAFF-1001 +2 faff-graft (Step 9b anchor-before-PR reorder: anchor+bundle+push inlined as numbered-list steps 4-5 before `gh pr create` step 6, In Review block relocated after the anchor/bundle detail, + the why-before-PR and narrow-nothing-to-commit rationale). renamed from SKILL_LINE_CAP_OVERRIDE (FAFF-584). FAFF-994 +1 beep-boop (durable judge-trail mint bullet at run-close, immediately after the anchor-run/bundle-publish bullets, not git-only-gated). FAFF-487 -7 gateway lines (Autonomous Mode Contract leaned: no-prompt/anti-park prose deduped + four preflights folded to one table; baseline lowered to lock the reduction). Sibling-skill invocation Producer-dispatch bullet later tightened (token-only, line-neutral). FAFF-948 +11 faff-graft (Step 3 reuse-arm staleness gate: `faff worktree-check` before "Skip to step 5" — fresh reuses as today, stale rebases onto the fetched base (via the JSON's worktree_path/base_ref, never an unestablished shell var) or parks on conflict; an exit-2 `reason:"no-worktree"` falls through to fresh-create, every other reason parks/warns instead of colliding with the already-checked-out worktree). FAFF-950 +3 beep-boop (turn-budget review-pending hold: the spec_review_turn_budget_pending ledger array entry + the "## Awaiting spec-review (slow fan-out)" run-summary subsection + the annotation twin). FAFF-954 +5 beep-boop (§4 build-queue gate: best-effort flag-guarded decision-capture record of the next/eligible core-loop decision) +5 faff-graft (Step 2 prep + eligibility gates: dispatched/autonomous-only decision-capture record of next/eligible). FAFF-895 +6 faff-graft (Interactive custody stamp sub-step broadened from L4 `--local`-only to L4 top-level `--local` or `--pr`; split its one over-cap paragraph into a bulleted lead + 5 bullets for the paragraph-word-cap lint). FAFF-842 +16 beep-boop (claim-before-admit: the endgame-only admission branch for a stranded In Review PR — reconstruct/store/PR/budget gates, recovery-claim mint/tick/release, landing_resumable ledger bullet + run-summary subsection) +1 faff-graft (Step 10 endgame-only entry paragraph: a third, no-rebuild dispatch shape re-entering the bounded landing loop directly). FAFF-889 -1 faff-graft (Step-5 build-claim mutex rewrite is net-leaner than the demoted breadcrumb prose it replaced — baseline lowered to lock the reduction). faff/faff-beep-boop: ratchet baselines, set to their exact committed size. faff-graft: unchanged carry-forward, see above. (FAFF-115 single-source gateway grows): FAFF-749 +2, FAFF-758 +2, FAFF-728 +1 gateway +14 beep-boop, FAFF-700 +7 gateway, FAFF-727 +10 gateway, FAFF-750 +2 gateway (concurrency obligation 7), FAFF-767 +1 beep-boop (surface intervention row) + faff-graft +2 (Step 10 dispatch-cut split + pr-ready return), FAFF-448 +6 gateway (decisions-register consult in Resolve-attempt before park) + faff-graft +6 (register-consult mirror + 3/5 bound-drift fix), FAFF-584 gateway ## Routing dedupe (line-neutral rename) + baseline re-pin to committed size, FAFF-699 +11 gateway (Interactive next-step offer subsection + Park-protocol recovery addendum), FAFF-761 +6 faff-graft (Step 3 standalone-interactive L2 mint block + Step 10 terminal outcome-write block), FAFF-214 +11 faff-graft (Step 9b draft→sanitize→check→gh pr create --body-file sequence + Step 8 pointer + autonomous-summary mirror), FAFF-782 +2 beep-boop (step 11.6 run-end synchronicity invariant — the whole reconcile→post-merge→owner-close tail runs in-turn, never a fire-and-forget background wait), FAFF-788 +1 beep-boop (Run ledger top-level level field-bullet), FAFF-779 +1 gateway (Park protocol (shared): conditional WIP/draft-PR steps + park-record accumulator + short-comment-rule) +1 beep-boop (park backstop's park_records accumulator write + summary faff-parks fence), FAFF-796 +1 beep-boop (git-only run-level anchor mint + commit bullet at orchestrator exit, per ADR 0109), FAFF-808 +5 gateway (Tracker availability resolution step 1: three-way probe branch for the git-only pin), FAFF-814 +12 beep-boop (step 7 eager containment-gated discovered-scope filing at L4 + step 8.0 reconcile/backstop + filed_this_wave accounting), FAFF-810 +1 beep-boop (step 3 re-slice-handoff prep-return bullet + reconciliation non-attach-expecting wiring), FAFF-841 +107 faff-graft (Step 9b In Review transition + Step 10 bounded landing loop: observe(pr) oracle, landing_loop procedure, landing-resumable outcome token + ledger-bucket mapping), FAFF-819 +4 faff-graft (Step 9b bundle-publish paragraph, after the anchor mint) +1 beep-boop (run-close bundle-publish bullet, after anchor-run), FAFF-817 +1 gateway (Slots table: transport row), FAFF-884 beep-boop line-neutral (Turn-survival invariant in-flight-marker sentence appended in-line; baseline unchanged at 739) + faff-prep +2 (spec-review-gate in-flight-marker turn-survival prose, under the uncapped 600 default, no baseline entry), FAFF-844 +63 faff-graft (Step 10 staged fix cycle: autofix config gate, handle_fixable_state/run_fix_cycle/same_class/observe_to_terminal, loop-entry hard-park-at-3 read, CONFLICTING/CI_RED_REGRESSION re-pointed via a CONTINUE sentinel resolved only at the SWITCH call site), FAFF-929 +10 faff-graft (Step 4c rewritten from presence-only to reconcile-before-materialise: `faff decisions intent-status` classify + skip-superseded/not-intent, a consistency backstop against the just-committed spec mirroring Step 4b/3b's detect_contradictions seam, skip+supersede+surface on an obsoleted intent, and a `faff decisions validate` pass after any materialise). FAFF-998 +11 faff-prep (new baseline entry — prep was previously on the shared 600 default; the per-round ratified tracker-resolution fold at the ratified-scope assembly seam adds the marker-gated/live/non-automation gather + the fail-closed `--fold-resolutions --into` fold, leaned via the CLI --into append so the SKILL carries no exit-0/3 synthesis bash). FAFF-1061 +15 faff-graft (Step 9 Interactive verification opt-in subsection: shell `faff verification resolve` for the per-leg attended-session opt-in, spec_review pins the full four-lens set, code_review is an advisory adversarial pass, holdout widens the Step-10 gate; plus the acting invariant that a floor result is reported never acted on; and the Step-10 Holdout-gate attended-advisory bullet with the cage-trust stamp and zero-born-verifiable no-op)
