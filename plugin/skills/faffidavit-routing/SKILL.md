...
| `needs-decision-first` | Spec contains an explicit `Punt:` / `needs human` / `TBD` / "or X if Y" marker that is **not** spec-closed, **or** the spec carries `confidence: medium` (the retained rating is itself the human-call signal — thin rationale or open punts) | Bounded resolve-attempt (gateway → **Resolve-attempt before park**); on failure, parks through the gateway **Park protocol (shared)** — root cause `punt-not-closed`, a short reason line naming the Punt + its `decides:` owner. This adaptor does not duplicate that label/comment/record procedure. |
