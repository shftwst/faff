...
**How this contract reaches you.** The fixed definition is loaded by the invoking consumer (`/faff-tidy` / `/faff-beep-boop` / `/faff-wtf` read the gateway on entry), so when you run as the `routing_adaptor` slot it is already in context. If you are invoked **standalone** ("why wouldn't SHF-123 fire?"), **Read the sibling `faff/references/kernel.md` (the shared kernel) → _Core contracts and adaptor slots → Automation-routing verdict_ now**, together with the lane references routing consumes — `faff/references/autonomous.md` (Resolve-attempt before park), `faff/references/methodology.md` (the methodology slot), and `faff/references/review.md` (Producer slots vs adaptor slots) — before assigning. Refer back to it; never treat the recap below as the source of truth.
