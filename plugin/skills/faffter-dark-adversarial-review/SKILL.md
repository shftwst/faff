...
- **Wire shape.** `review-call.mjs` places the shared context+diff block in the cacheable **prefix** position (the builders' `system` slot: `messages[0]` for ollama/OpenAI, top-level `system` for Anthropic) and the `--system` review lens in the trailing `user` turn. The flag names are unchanged; only the wire ordering flips so a prefix-caching backend can reuse the shared prefill. Code review is a single call with no sibling to share a prefix with, so the caching win lands on spec-review's four-lens fan-out, which shares this transport, not here.
