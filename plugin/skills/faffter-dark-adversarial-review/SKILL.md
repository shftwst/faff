...
- **`adversarial_outcome` is OPTIONAL and additive** — include it **only** in the autonomous full-chain-outage case, set to `"chain-outage-skipped"` (see **Full-chain outage annotation**); omit it on every other path. The contract validator reads only `signal`+`findings` and ignores unknown fields, so the field rides transparently through `faff contract review-verdict` and is consumed by the beep-boop orchestrator — it never alters the `pass`/`fail`/`needs-human` verdict.
