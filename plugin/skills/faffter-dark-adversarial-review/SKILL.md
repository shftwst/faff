...
- **An over-window skip is a config fault, so it counts as one in terminal precedence.** A declared window too small for your diffs is actionable operator config, not a transient outage, so it joins the existing config-fault classes under the no-silent-weakening rule: a fully-failed chain containing one surfaces `needs-human` (exit 2), where a chain of purely availability failures would have been `pass`+skip (exit 5). Concretely, a guard-skipped primary followed by unreachable fallbacks now terminates `needs-human` rather than pass+skip. That is the stricter direction and it is deliberate: silently skipping review because the configured windows cannot fit any payload is exactly what this feature exists to stop being silent.
...
| `2` | usage error, a genuinely unknown provider (not one of ollama / openai / vllm / openrouter / nvidia / deepseek / gemini / anthropic), **or** a non-transient **request fault** (e.g. HTTP 400/413) that escaped a backend's orchestration function and was caught at the fallback-chain boundary, **or** the assembled review payload (system + context + diff) exceeded the oversized-diff preflight's size threshold and was flagged *before* any backend was dispatched (zero network calls attempted), **or** a backend was **deliberately** advanced past because the estimated payload did not fit its declared `context_window` — a skip decided before dispatch, not a fault, and always surfaced via `PRIMARY_SKIP_SIGNAL` when it was the primary — a fully-exhausted chain containing only that request-fault class also terminates here, never `5`/pass+skip | **`needs-human`** — a config/request fault, not a review result. | same |
