...
- **exit `0`** — `$backends_json` holds the primary-first JSON array `review-call.mjs`'s `--backends-json` mapper consumes verbatim (`{provider, model, host, api_key_env?, reasoning_off?, timeout?}`). Invoke `review-call.mjs --backends-json` and disposition its own exit per the **Exit code → Phase-2 outcome** table below.
