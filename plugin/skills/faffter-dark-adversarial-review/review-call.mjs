...
// --- FAFF-1051 / FAFF-1058: the prose diff kind -------------------------------------------------
//
...
// recovers no anchors from prose, so any reduction is head truncation, not relevance. A spec plus the
// files it names has no anchors to keep, so a partial spec review is worse than an honest over-window
// skip. FAFF-1058 therefore makes the "prose" kind a true pass-through: `trimContextFiles` returns the
// context unchanged (reason "prose-passthrough") and main() skips the FAFF-1039 window pass for prose,
// so both passes are no-ops at any size. Every silent caller stays on "unified", byte-for-byte unchanged.
...
// This is the anchored search only. main() never calls it for a prose diff (FAFF-1058 skips the whole
// FAFF-1039 window pass under the prose kind), so there is no head-only regime here.
export function tightenToTarget({ contextFiles = [], diff = "", targetBytes, assembleFn = assembleUserMessage } = {}) {
  ...
  let best = null;
  for (const rung of TRIM_WINDOW_LADDER) {
    const { contextFiles: trimmed } = trimContextFiles({ contextFiles, diff, thresholdBytes: 1, window: rung, diffKind: "unified" });
    ...
    if (targetBytes == null || contextBytes <= targetBytes) {
      return { contextFiles: trimmed, prefix, bytes, contextBytes, window: rung, headLines: DEFAULT_TRIM_HEAD_LINES, mode: "anchored", fitted: true };
    }
    // The min is tracked on PREFIX bytes, because the prefix is what gets sent.
    if (best === null || bytes < best.bytes) best = { contextFiles: trimmed, prefix, bytes, contextBytes, window: rung, headLines: DEFAULT_TRIM_HEAD_LINES };
  }
  return { ...best, mode: "anchored", fitted: false };
}
...
export function trimContextFiles({
  ...
  diffKind = "unified",                                  // FAFF-1051: "unified" | "prose"
} = {}) {
  ...
  // Gate: disabled (0) means no reduction at all, checked BEFORE the kind so it holds under both.
  if (!(thresholdBytes > 0)) {
  ...
  // FAFF-1058: prose is a true pass-through at any size. A spec plus the files it names has no diff
  // anchors, so any reduction would be head truncation; a partial spec review is worse than an honest
  // over-window skip. This branch returns the input unchanged; main() also skips the FAFF-1039 window
  // pass for prose, so both trim passes are no-ops.
  if (diffKind === "prose") {
    return { contextFiles, report: { trimmed: false, reason: "prose-passthrough", kind: "prose", hunks: 0,
      headLines: null, filesTotal, bytesBefore, bytesAfter: bytesBefore } };
  }
  ...
  return { contextFiles: out, report: { trimmed: true, reason: "trimmed", kind: "unified", hunks,
    headLines: null, filesTotal, bytesBefore, bytesAfter } };
}
...
export function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (k === "--no-trim") a.noTrim = true;   // FAFF-1058: disable BOTH trim passes (pass 1 → disabled short-circuit, skip the FAFF-1039 window pass). Absent ⇒ behaviour byte-for-byte unchanged.
...
export async function main(argv, { runReviewFn = runReview, checkFn = realCheck } = {}) {
  ...
  // FAFF-1051: `diffKind` (resolved above) selects the kind; "prose" is a pass-through (FAFF-1058).
  // FAFF-1058: --no-trim drives pass 1 to its disabled short-circuit (thresholdBytes 0) AND skips the
  // FAFF-1039 window pass below, so both passes are no-ops.
  const noTrim = a.noTrim === true;
  const { contextFiles, report: trimReport } = trimContextFiles({
    contextFiles: contextFilesRaw,
    diff,
    thresholdBytes: noTrim ? 0 : (a.contextTrimBytes === undefined ? DEFAULT_CONTEXT_TRIM_BYTES : a.contextTrimBytes),
    diffKind,
  });
  ...
  // FAFF-1058: skipped entirely for the prose kind (a spec never trims) and when --no-trim is set. The
  // `diffKind !== "prose"` clause alone excludes prose; `noTrim` is the operator opt-out. Either way the
  // per-backend guard still handles an over-window payload as an honest skip.
  const primaryWindow = chain.length ? chain[0].contextWindow : null;
  if (primaryWindow && !noTrim && diffKind !== "prose") {
    ...
    if (estWithReserve > usable) {
      ...
      const rungDesc = `trim-window ${tightened.window}`;
