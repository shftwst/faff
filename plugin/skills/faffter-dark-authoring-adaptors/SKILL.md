...
| `spec` / `review` / `ship` (producers) | Spec readiness / Review verdict / Delivery outcome — the producer **emits** a `faff-contract:<name>` block (`spec-readiness` / `review-verdict` / `delivery-outcome`) the consumer parses (FAFF-109); no paired adaptor | how the work is done + the self-declared contract block; a producer that can't emit it is wrapped here |
