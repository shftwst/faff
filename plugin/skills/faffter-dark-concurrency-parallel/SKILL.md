...
> **Heartbeat ownership under N concurrent subagents is unresolved** — with more than one active writer the single-active-writer property the sequential default relies on does not hold (`owner.last_heartbeat`). This is a known punt for the parallel executor; resolve it before enabling subagent dispatch under parallel mode in anger. The sequential default is unaffected.
