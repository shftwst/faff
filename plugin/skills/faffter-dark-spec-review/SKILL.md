...
{ "lens": "infosec", "outcome": "refuted|clear|unavailable",
  ...
  "objections": [{ "severity": "critical|major|minor|observation", "claim": "…", "evidence": "…", "predicted_consequence": "…", "summary": "…" }],
  "model": "provider/model" }
```

Each objection carries the **enrichment triple** `{claim, evidence, predicted_consequence}`, parsed from the lens's `### [severity]` block. `aggregate.mjs` carries these verbatim onto the output objection, so they survive into the round record and thence `faff spec-judge-evidence`. A taste-level objection that cannot name a concrete consequence sets `predicted_consequence: "not separately stated"`. The triple is **additive** — it never changes the majority/severity gate.
