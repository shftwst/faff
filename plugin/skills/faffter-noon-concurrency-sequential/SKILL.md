...
**Heartbeat ownership follows the single active writer.** Because the orchestrator blocks awaiting the subagent, exactly one of them is active at any instant: between dispatches the orchestrator refreshes `owner.last_heartbeat`; during a dispatched build the subagent does (it has `run_dir`/`session_id`). No concurrent writers, no race — `runcheck` never sees a live build as abandoned. This preserves the FAFF-205 owner stamp across the whole graft lifecycle.
