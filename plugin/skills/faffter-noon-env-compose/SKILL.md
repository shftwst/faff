...
- **Honour the recommendation.** On any non-build recommendation (canonical semantics: `faff contract architecture-proposal --describe`), provision nothing: surface the proposal for a human and emit no handle. Procurement is a separate, out-of-scope concern.
- **Resolve the transport.** Resolve `faff config get slots.transport` (default `faffter-noon-transport-private-network`; resolve via `config get`, never name the default literally) and invoke it inline with the provision context (`evaluator_topology`, `substrate`), receiving `{ base_host, credentials?, teardown? }`. With no separated evaluator in the provision context (`evaluator_topology` `"co-resident"` or absent), the transport resolves `base_host = "localhost"` — the deterministic, byte-identical default.
- **Decide the subject(s) under test + how to reach them.** Per issue, weigh the spec/DoD + the architecture proposal + the infra profile to decide what this holdout must exercise and how each subject is best reached: a running **service** (the compose path below), a **code component** reached via the Node reflection bridge (`faff-bridge-node.mjs`) (the code-interface branch below), or **both** — service-vs-code-interface is not binary, and a project that composes as services will also have code-only components worth reaching over the bridge. A committed `.faff/binding-manifest.json` is the bridge's *input*, not the trigger — do not branch on its mere presence. (This is a provisioning choice, the same class this occupant already makes over datastores and the recommendation gate; its calibration as a graded judgement seam is a follow-up.)
- **Code-interface branch (for each component reached via the bridge).** Resolve `faff config get slots.env-code-interface` (default `faffter-noon-env-code-interface`; resolve via `config get`, never name the default literally) and invoke it inline with the component, the resolved `base_host`, the manifest path (`.faff/binding-manifest.json`), and `evaluator_topology`. It builds the component, stands up the bridge (`faff-bridge-node.mjs`) as a health-checked service **in this env's own compose project**, and returns `{ ok, endpoint, health_checks[], teardown_ref, teardown_cmd, provisioner, notes?, violations? }` (a plain object, no gated block). On `ok:false`, emit a `status: failed` handle with its `violations` and stand up no half-up env. On `ok:true`, fold it into the one handle this occupant emits: add `endpoint` to `endpoints[]` (keyed by subject; the top-level `endpoint` is the primary subject), append its `health_checks[]`, and — because the bridge shares this env's compose project — the existing `teardown_ref`/`teardown_cmd: "faff env down --project <name>"` tears down services *and* bridge together. Reused verbatim: the resolved `base_host` (fed to the bridge instead of `compose-gen`) and `envValidateBaseHost`. The bridge runs where `faff env up` runs (SUT-cage side); proving that placement is the downstream end-to-end conformance test.
- **Generate the plan with the resolved base** (service subjects) — `faff env compose-gen --profile <profile> --base-host <base_host>` writes the compose file and prints a deterministic **ProvisionPlan** (`services`, `endpoint`, `endpoints`, `health_checks[]`, `seed_targets[]`, `unprovisionable[]`). This is the single source for the handle's fields — copy them from the plan, never re-derive in prose. `endpoint`/`endpoints` are **resolved absolute** URLs derived from a per-service relative surface (scheme, host-published port, path) against the resolved base host — the default output is byte-identical to `localhost`, and the transport re-bases the same surfaces off-host without changing the handle's shape. The base is validated as a bare host/IP (no scheme/port/path/query/fragment/userinfo; IPv6 bracketed) via `envValidateBaseHost` and fails loud otherwise, emitting no compose file; `surfaces`/`base` are producer-internal plan fields, never copied into the handle. When the repo ships its own compose declaring a single buildable app, the CLI reconciles the generated app service against it (port, env, build context/dockerfile, healthcheck — the real contract wins over the coarse profile) and the datastore tier too (the datastore's auth env is lifted so the app's wired role/db exist, and the app is ordered behind it with a `service_healthy` gate); with no repo compose the synthesis is unchanged.
...
- **Emit the ready handle** only once the env is up, healthy, and seeded: `status: ready` with the plan's `endpoint` / `endpoints` / `health_checks[]`, the plan's `project_name` as `teardown_ref` + `teardown_cmd: "faff env down --project <name>"`, dev/test `credentials` in-block, `provisioned_at`, `provisioner`, and any plan `notes` (e.g. an unseeded `mount` store) carried through. If the transport returned `credentials`, carry them in this same `credentials` object; if it returned a `teardown`, fold its `ref`/`cmd` into `teardown_ref`/`teardown_cmd` so the one teardown removes the env *and* any transient transport artifact together — the handle shape gains no new field either way. The env is **ephemeral by default** — one env per evaluation run, torn down on completion via `faff env down`.
...
A `ready` handle must carry a non-empty `endpoint`, a non-empty `health_checks[]`, and a `teardown_ref`; any other status (canonical semantics: `faff contract env-handle --describe`) is a well-formed non-ready state the contract reports at exit 1 (not gate-passing). A swapped-in producer (a cloud-preview or persistent-staging strategy) conforms by emitting the same block.
