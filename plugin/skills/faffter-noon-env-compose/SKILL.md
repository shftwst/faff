...
- **Generate the plan** — `faff env compose-gen --profile <profile>` writes the compose file and prints a deterministic **ProvisionPlan** (`services`, `endpoint`, `endpoints`, `health_checks[]`, `seed_targets[]`, `unprovisionable[]`). This is the single source for the handle's fields — copy them from the plan, never re-derive in prose. When the repo ships its own compose declaring a single buildable app, the CLI reconciles the generated app service against it (port, env, build context/dockerfile, healthcheck — the real contract wins over the coarse profile); with no repo compose the synthesis is unchanged.
