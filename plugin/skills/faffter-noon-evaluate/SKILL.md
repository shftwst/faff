...
- **Roll up + emit.** Derive the `aggregate` from the criteria (any `needs-human` or empty → `needs-human`; all `met` → `meets-spec`; all `unmet` → `fails`; else `gaps`), set `code_blind: true`, and emit the block. Write the human-readable evidence as a prose report alongside (not in the block), and persist the block to `.faff/holdout/<issue|run>.json` — `faff holdout verdicts` reads that store, re-validates each block through the same gate, and feeds the resulting `{prdr: met}` map to `faff prdr coverage --dod-verdicts` for the `prd-satisfied` roll-up.
