...
The default **producer** for the `ship` slot. Delivers a PR that `/faff-graft` has already cleared through the **integrity floor** (AC-verified + CI-green + review `pass`): runs a deploy-readiness check, merges, and cleans up what it created — then emits a native delivery result **plus a `faff-contract:delivery-outcome` block** that faff-graft Step 10 parses (via `faff contract delivery-outcome`) onto the fixed outcome vocabulary graft routes on (canonical semantics: `faff contract delivery-outcome --describe`). The safe, zero-config default — a no-op readiness check and a merge through the `faff merge-gate` interlock (never a raw `gh pr merge`, FAFF-350), no deploy step. Swap to a deploy-capable producer (e.g. `gstack:land-and-deploy`) when delivery means more than a merge.
