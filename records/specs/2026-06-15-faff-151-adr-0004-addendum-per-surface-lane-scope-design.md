...
This spec is for the build agent and human reviewers. It defines a **documentation-only** change: append a scoping addendum to `records/adr/0004-judgement-evals-spike.md`, plus a one-line forward-note on `records/adr/0003-live-driver-spike.md`. No code, no `eval/` behaviour changes.
...
|---|---|---|
| `records/adr/0004-judgement-evals-spike.md` | Markdown ADR | The record being amended (the addendum) |
| `records/adr/0003-live-driver-spike.md` | Markdown ADR | Left the three lanes open; gets a one-line forward-note |
| `eval/cli-driver.mjs` | JS | The black-box driver whose "doesn't run the skill" behaviour is the finding |

**Scope statement.** A documentation change to two ADR files under `records/adr/`, both append-only.
...
PROCEDURE add_addendum:
  1. Re-read records/adr/0004-judgement-evals-spike.md (confirm unchanged since prep).
...
PROCEDURE add_0003_forward_note:
  4. Re-read records/adr/0003-live-driver-spike.md.
...
Non-functional assertion: **no files outside `records/adr/0004-judgement-evals-spike.md` and `records/adr/0003-live-driver-spike.md` are modified; both diffs are purely additive.**
...
- **Assumes:** `records/adr/0004-judgement-evals-spike.md` and `records/adr/0003-live-driver-spike.md` are unchanged since prep read them. *Validation:* re-read both (and check `git status`) before editing; if `## Costed follow-ups` moved, re-locate the 0004 insertion point.
...
**Integration smoke test:** `git diff --stat` shows exactly two changed files (`records/adr/0004-…`, `records/adr/0003-…`); both diffs are purely additive (no deletions in prior sections).
