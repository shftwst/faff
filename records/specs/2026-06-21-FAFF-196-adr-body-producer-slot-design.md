...
A new `faffter-noon-adr` producer skill (written to the `docs/reference/skill-authoring.md` charter). Graft Step 4b resolves the `adr` slot and delegates body authoring; the `faff adr` CLI still owns scaffold + numbering + commit.
...
- [ ] `faffter-noon-adr` SKILL.md exists (`user-invocable: false`), documents emitting a Nygard ADR body + advisory confidence, carries **no** `faff-contract:` block, written to the `docs/reference/skill-authoring.md` charter; documented as the single ADR-authoring producer FAFF-27 reuses.
