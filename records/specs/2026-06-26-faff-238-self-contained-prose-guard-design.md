...
**The load-bearing model — directional self-containment.** Prose a reader *executes* (a skill prompt) or *publicly consumes* (a user guide) is the artifact, and the reader has no tracker access. A ticket or ADR is the *reasoning that produced* a rule — upstream provenance — never something the reader follows. So everything **inlines *from*** the reasoning corpora (`records/adr/`, `records/specs/`, tickets); **nothing in executed/public prose points *into*** them. A `FAFF-NN`/`ADR NNNN` ref in enforced prose is therefore always one of two defects: *decorative* (delete) or *smuggling meaning* the prose should state outright (inline the rule, then delete). There is no third "load-bearing reference" category in this prose.
...
- **The ADR corpus is exempt and must stay so.** `faff adr validate` *requires* symmetric supersession back-refs; banning ADR citations under `records/adr/` would break it.
...
| **External-artifact ref** | A `FAFF-NN` ticket tag, an `ADR NNNN` citation, or a numbered `records/adr/NNNN-*` pointer in enforced prose. |
| **Reasoning corpus** | `records/adr/`, `records/specs/`, tickets — where refs belong; enforced prose inlines *from* here. |
...
ADR_PTR  := /\bdocs\/adr\/\d{3,4}[-\w]*/        # records/adr/0013-...  (numbered pointer only)
```
...
- **Chosen:** match *numbered* artifacts only — a bare `records/adr/` path naming the directory the `faff adr` command operates on is **not** a violation; only a numbered `records/adr/NNNN-*` pointer is.
...
- **Chosen:** `records/adr/**` is exempt (forced by `faff adr validate`).
...
**`--selftest`** drives an in-memory table: `FAFF-238` → ticket hit; `ADR 0013`/`ADR-9` → adr-cite hit; `records/adr/0013-foo.md` → adr-ptr hit; `gateway → Section`, `faff/SKILL.md`, bare `records/adr/` mention, `faffter-dark-nlspec` → **no** hit.
...
**Standard rewrite — `docs/reference/skill-authoring.md`:** replace the `stray marker` row's parenthetical and the "Honest limits … deliberately narrow … load-bearing FAFF-NN anchors" sentence with the forward rule: *executed/public prose (`SKILL.md` + `docs/guide/`) carries no `FAFF-NN`/`ADR` ref — state the rule, inline from the reasoning corpus; `docs/` outside `docs/guide/` is ref-permitted; within-prose anchors are not external refs.*

**CI wiring:** add `faff lint-refs` + its `--selftest` to `validate.yml` beside `validate-adapters` and `node --test`.

**Edge cases:**
- Bare `records/adr/` (no number) → **not** a violation.
...
- **Anti-pattern:** banning a bare `records/adr/` directory mention. **Why:** legitimate command description, not a provenance pointer.
...
```
```
Given docs/guide/architecture.md citing "ADR 0010" and "records/adr/0010-foo.md"
...
```
```
Given records/adr/0013 "Superseded by ADR-0014" and 0014 "Supersedes: ADR-0013"
When  faff lint-refs runs AND faff adr validate runs   Then lint ignores records/adr/** AND adr validate exits 0
```
...
**Match numbered artifacts only?** **Chosen:** yes — a bare `records/adr/` mention is legitimate description; only numbered pointers/citations are provenance.
...
**From WHY**
- [ ] A `FAFF-NN` / `ADR NNNN` / numbered `records/adr/NNNN-*` ref in any `docs/guide/**/*.md` fails the check, naming `file:line ✗ match`.
- [ ] `docs/` outside `docs/guide/` is never flagged; a bare (un-numbered) `records/adr/` mention is not a violation.

**From WHAT**
- [ ] The three patterns match `FAFF-238`, `ADR 0013`/`ADR-9`, `records/adr/0013-foo.md`; they do **not** match `gateway → Section`, `faff/SKILL.md`, bare `records/adr/`, or `faffter-*` names.
...
- [ ] `docs/reference/skill-authoring.md` rewritten — false "load-bearing FAFF-NN anchors" carve-out removed; ban stated forward.
