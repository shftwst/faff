...
| The as-built picture | `docs/reference/architecture/l4-container-permission-model.svg` + `docs/reference/architecture/l3-l4-architecture.md` (:18–34, :43–45) | Already draws one claude-box, context-isolated lanes, the FAFF-276 gap, and the outer HOST (laptop · CI runner · factory compute) with a human at the launch point |
...
- **ADR + refutation log** → committed on this ticket's branch, shipped by PR (`records/adr/00NN-…` + `records/spikes/2026-MM-DD-FAFF-313-…-refutation-log.md`).
