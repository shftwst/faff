...
| ADRs 0034 / 0036 / 0037 | `records/adr/` | Dispatch-boundary kill-switch intent; lights-out runner; level-scoped-appetite precedent for a level-scoped default |
...
- `docs/reference/architecture/l3-l4-architecture.svg`: the L4 governance badge already states this model — verify it matches the shipped behaviour; the L3 badge (`at_ceiling: stop`) stays.
...
- All five `verification/external-verification/scaffold-p*.sh` configs pass the new preflight unmodified (each already sets `budget.tokens`).
