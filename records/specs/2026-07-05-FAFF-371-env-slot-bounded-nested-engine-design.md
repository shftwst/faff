...
| `records/adr/0041-…` decision 3 | Markdown | The boundedness criterion this work satisfies |
...
### Docs (same PR)
- [ ] `docs/reference/cage-engine-acceptance.md` exists, encoding the four-point joint acceptance shape with faff's three points as concrete commands and point four marked external.
...
- The gap: Assume 1 (claude-box provides a docker-compat socket + compose plugin) is a load-bearing cross-repo dependency that exists only in spec prose and a human comment. The joint runbook's full 4-point acceptance cannot complete without claude-box shipping point 4, and no tracker artefact encodes that. **Closed 2026-07-05, recorded 2026-07-26:** FAFF-381 ("Cage-engine acceptance run in anger") was filed the same day and carries the `blockedBy` edge on FAFF-380 (the claude-box delivery) that this paragraph asks for. FAFF-380 completed 2026-07-11, so FAFF-381 is unblocked; it remains in backlog awaiting an in-cage run.
