...
| `records/adr/0042-three-tier-region-model…` | The region model (shared-infra / governance / factory / shell) whose banners pre-tag the split; predicts the lint retires post-modularisation |
...
**Docs sweep (same PR):** `docs/guide/cli.md:3` and `plugin/skills/faff/SKILL.md:96` lose the "a single dependency-free Node script" phrasing in favour of accurate wording (dependency-free Node CLI: one entrypoint plus modules under `bin/lib/`, still no npm/install/build); `plugin/skills/faff/SKILL.md:110` "one Node entrypoint for all bundled helpers" stays true and stays put. The entrypoint's own header comment (lines 26–28, "Just a Node file") gets the same accuracy sweep — comments are not behaviour. Historical `records/specs/*` and ADR 0042 are append-only provenance: untouched; ADR 0042's status handling rides the regions-retirement follow-up ticket.
