...
| `plugin/skills/faff-graft/SKILL.md` (L438, ADR-collision renumber) | prose | `git add -A records/adr/` — already path-scoped; lower risk but in the same class. |
...
**D. ADR-collision renumber (graft L438).** Already path-scoped (`git add -A records/adr/`). Tighten to add only the specific renumbered ADR file(s) by explicit path and run `stage-guard --mode assert` before commit. Lower-risk, folded in for consistency.
