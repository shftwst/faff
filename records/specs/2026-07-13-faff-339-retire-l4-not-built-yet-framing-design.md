...
**Problem statement.** The gateway's L4 row and bullet still say **"The frontier. Not built yet, mind."** `faff lights-out` shipped: it is the enforced L4 entry point today (`plugin/skills/faff/bin/lib/lights-out.js`; `docs/guide/unattended.md:36–48`). The stale hedge is a drift defect (audit finding **R1** in `verification/audits/2026-07-04-faff-323-whole-system-coherence.md`) — an autonomous decision keyed off "L4 isn't real yet" would be wrong, and a human reading the gateway is told a shipped capability is vapour.
...
| `verification/audits/2026-07-04-faff-323-whole-system-coherence.md` | Markdown | Audit finding R1/T5 that motivates the change. |
