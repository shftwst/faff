...
- **Legacy-lenient.** The ~66 existing ADRs (`records/adr/`, none carrying Provenance) must keep passing `faff adr validate` untouched.
...
| `records/adr/0022-…` | prose | The two-gate admission decision being ported |
...
- `faff adr validate` over the untouched existing `records/adr/` tree MUST report zero problems after this change.
...
- [ ] `faff adr validate` on the existing `records/adr/` tree (no Provenance anywhere) reports zero problems; a file with `Provenance: robot` is flagged
