...
| `verification/external-verification/` | scaffolds | the git-only SUTs (FAFF-310) that exercise this end-to-end |
...
- **Assumes:** the git-only SUTs declare engineering gates that `faff gates run` discovers (pre-commit / `package.json` scripts / `Makefile`). *Validate:* run `faff gates run --json` in a scaffolded SUT (`verification/external-verification/scaffold-p1-*.sh`) and confirm `discovery` ≠ `none`; if a SUT legitimately has none, the `no-ci-coverage` → fail-closed refusal is the correct (not accidental) behaviour.
...
PROCEDURE smoke_git_only_merge():
  1. Scaffold a no-remote SUT (verification/external-verification/scaffold-p1-*.sh); make a clean feature branch.
