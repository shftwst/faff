...
| `verification/external-verification/scaffold-p{1,2,3}.sh` | bash | Surface 2 — L4-eligible SUT scaffolders; emit `review`/`spec_review` slots but **no** backend config |
| `verification/external-verification/scaffold-p{4,5}.sh` | bash | Out of scope — git-only, gated, `review: faffter-noon-review`; no adversarial backend |
...
| `records/adr/0067-committed-config-posture-two-file-model.md` | ADR | The two-file (base + `.local`) posture this migration honours; names the tailnet host as an overlay *candidate*, not a mandate |
...
FAFF-523 landed with a **different reference-list field name** than this spec's placeholder assumed. The spec's §3/§4 placeholder used `models.review_adversarial: [name, ...]` as the pointer form; FAFF-523's actual landed schema (confirmed in `plugin/skills/faff/bin/lib/backends.js`, `plugin/skills/faff/bin/lib/adversarial-backends.js`, and FAFF-523's own spec `records/specs/2026-07-16-FAFF-523-shared-modelproviderauth-backend-config-namespace-design.md`) uses **`faffter_dark.adversarial: { refs: [name, ...], deadline?: N }`** — an ordered reference list living directly under the (slimmed) `faffter_dark.adversarial` key, not a `models.*` pointer. `models.review_adversarial` was only ever a speculative mention in an earlier, unrelated design doc (`2026-07-03-FAFF-315-per-lane-model-selection-design.md`) and was never implemented.
...
## Build-time correction: Surface 1 (faff base `.faffrc.yaml`) was already migrated by FAFF-523 (2026-07-17)

A second, more significant discrepancy from this spec's premise: **FAFF-523's own landed commit (4c0179c, PR #408) already migrated faff's own `.faffrc.yaml`** to the `backends:` namespace as its documented proof-of-concept slot migration (matching this ticket's own WHY section: "523's proof does the mechanical adversarial migration; this completes it"). This spec's "Surface 1" procedure (§4 `migrate_faff_base`) was written as if that migration were still outstanding.

**How this was caught.** The build's worktree-setup step copies gitignored overlay files (`.env*`, `.faffrc.local.yaml`) from the main checkout into the new worktree — a step meant for machine-local overlays that never enter git. `.faffrc.yaml` was included in that copy list too, and the main checkout at branch time happened to be pinned to a commit *older than* FAFF-523's merge — so the copy silently overwrote the correctly-checked-out (already-migrated) `.faffrc.yaml` with a stale, pre-523 version carrying the legacy `faffter_dark.adversarial` scalar block. The build proceeded against that clobbered file, "re-migrated" it, and in doing so **reversed a considered, documented decision FAFF-523 had already made**: 523's migration deliberately left `auth`/`egress` implicit (derived from `api_key_env`/host) rather than explicit, and deliberately did **not** restate the legacy resolver's spurious `NVIDIA_API_KEY` inheritance onto the keyless `studio-ollama` entry — its own commit comment names this exact quirk and rejects perpetuating it ("Migration deviation ... NOT restated here; studio-ollama correctly derives auth: none").

**Correction applied.** `.faffrc.yaml` is reverted to origin/main's content verbatim (no further change) — Surface 1 is already done, and 523's own field-value choices stand un-relitigated. Surface 2 (the P1/P2/P3 SUT scaffolders) is unaffected by this bug — those files are not part of the config-copy list, were correctly checked out from origin/main, and genuinely still carried the legacy scalar block (no `backends:` at all) as the spec's WHY section described. That is FAFF-529's real remaining scope, and where this build's substantive changes now live.

**Follow-up (recorded as discovered scope, not fixed here):** the worktree-setup script's unconditional `.faffrc.yaml` copy is a latent footgun whenever a worktree is branched from a main checkout that is behind the true base ref — it silently reintroduces stale *tracked* config under the guise of restoring a gitignored overlay. `.faffrc.yaml` is tracked in this repo (not gitignored), so it should never need copying — the branch's own checkout already carries the correct committed content; only genuinely gitignored files (`.env*`, `.faffrc.local.yaml`) need the copy.
