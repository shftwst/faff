...
| `records/specs/2026-07-16-FAFF-199-adr-l4-loop-authored-adrs-mutable-means-design.md` | Markdown (design doc) | The supersession-authority axis this ticket does NOT touch |
| `records/specs/2026-07-15-FAFF-342-adr-status-sweep-design.md` | Markdown (design doc) | The one-time historical sweep + the non-gating "Accepted cites Proposed" advisory this ticket keeps sound going forward |
| `records/specs/2026-07-07-FAFF-368-adr-renumber-merge-gate-design.md` | Markdown (design doc) | The `git diff --diff-filter` mechanism this ticket's ADR-id derivation reuses verbatim |
...
### From HOW (behaviour)
- [ ] Step 10's new ADR-accept sub-step derives this run's touched ADR(s) via `git diff --diff-filter=AM <merge-base>..HEAD -- records/adr/`, matching FAFF-368's existing mechanism
...
PROCEDURE smoke_test_accept_after_renumber():
  ...
  5. Assert: `faff adr accept` is invoked against records/adr/0051-x.md (NOT
