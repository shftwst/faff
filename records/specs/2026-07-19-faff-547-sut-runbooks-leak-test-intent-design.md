...
|---|---|---|
| `verification/external-verification/scaffold-p{1..4}-*.sh` | Bash (heredocs) | The four leaking scaffolders this spec edits |
| `verification/external-verification/scaffold-p5-brownfield.sh` | Bash | Already immune — pastes a literal vague ask, never `BRIEF.md`; untouched |
| `test/scaffolder-lights-out-dials.test.mjs` | Node test | Static heredoc lint; the new paste-hygiene guard extends it |
| `verification/external-verification/README.md` | Markdown | Documents the per-SUT file layout + paste flow (L34–48); needs updating |
...
**From docs**
- [ ] `verification/external-verification/README.md` (L34–48) is updated to describe the new paste flow and the `BRIEF.md`-role overload: PRD-less SUTs paste the neutral `BRIEF.md`; PRD-backed SUTs paste the PRD and treat `BRIEF.md` as operator-only; operator framing lives in the RUNBOOK.
