...
- **One home, referenced not copied.** The policy is shared prose, so it gets exactly **one** canonical home (the gateway); the existing level-coupled sites reference it, they do not restate it (repo authoring standard — `docs/reference/skill-authoring.md`; CLAUDE.md dedup rule).
