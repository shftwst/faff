...
- **One source per schema.** No schema is duplicated between `verification/evidence/` and the runtime; where the runtime has no separately-schema'd artifact (the case here — `chain-head.json` has no `contracts/*.schema.json` twin), v0.2 authors one new schema.
...
PROCEDURE build_the_v0.2_delta:
  1. Scaffold verification/evidence/v0.2/ (conformance.md, anchor-integrity.md, schema/)
  ...
  5. Update verification/evidence/README.md: index gains the v0.2 row; versioning-policy changelog
...
When a reader opens verification/evidence/README.md
...
### From WHY
- [ ] `verification/evidence/v0.2/` exists alongside an untouched `verification/evidence/v0.1/`; every statement in v0.2 describes shipped behaviour (no aspirational fields).
...
- [ ] `verification/evidence/README.md` gains a v0.2 index row and a dated changelog entry naming FAFF-568/FAFF-601.
