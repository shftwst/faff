...
- **Editing historical `records/specs/*.md` that mention `--execute`** — frozen records of what was built; they are evidence here, not targets.
...
- Option (b) — strike `--execute` from bin/faff:157, merge-gate's usageError, cli.md, and graft SKILL.md Step 10. Pros: smaller vocabulary. Cons: edits four-plus live surfaces, breaks the `[--execute|--check-only]` symmetry operators expect, cannot fix the frozen records/specs or human muscle memory, and contradicts FAFF-537's recorded intent.
