...
| `records/spikes/2026-07-10-faff-411/` | A spike committed as a directory: its own code, its raw machine output, and `RESULTS.md` | The layout this ticket's findings record follows |
...
**Chosen:** from `/proc/1/environ`, emit key *names* only plus the full value of the `container` key, behind an `--environ-keys=names|count` flag defaulting to `names`. The key-name enumeration is itself a disclosure surface the prefix scan is blind to by design. On a hosted runner pid 1 is the VM's init, so names are safe and are the useful reading. **On a self-hosted host that argument does not hold** — pid 1 may be the runner service rather than the VM's init, and what its environment holds is not something this ticket can measure.

That makes the default *shape-scoped*, and the instrument is not: FAFF-656 reuses it unchanged on exactly the shape where the safety argument fails. **So FAFF-656 must choose the mode deliberately before its first reading, and record the choice**, rather than inheriting `names` because it is the default. The flag means it restricts by argument rather than by editing the instrument, which is what keeps "same instrument" true.

Worth stating precisely, because it is easy to overstate: a key **name** is not a credential. No key's **value** is emitted by that enumeration — the one environ value in the record is the `container` key's, exception 3 below, which is emitted in both modes — so the exposure is which variables exist, not what they hold. But the pre-commit scan cannot catch a secret-*shaped* name either — a block continuation line carries no `key: value` separator, so the generic branch never reaches its value test, and a name matching no known prefix passes clean. The defence for this signal is the mode flag and a human reading the block before commit, not the scan.
...
```
records/spikes/2026-07-26-FAFF-654/
...
- No committed file under `records/spikes/2026-07-26-FAFF-654/` MUST match any of the three scan branches ported from `config.js:870–880`.
...
**Chosen:** `records/spikes/2026-07-26-FAFF-654/` following FAFF-411's layout, `RESULTS.md` without a `Headline` section.
...
**Assumes:** the image chosen for the containerised job carries what the probe needs *and what the job needs* — a POSIX shell and every command the probe or these steps call **unguarded, or whose absence would degrade a header reading** — `sh`, `ls`, `id`, `cksum`, `dd`, `mktemp`, `wc`, `tr`, `cut`, `sed`, `sort`, `head`, `grep`, `date`, `uname`, `dirname`, `cat`, `chmod`, `mkdir`, `ln`, `rm` — plus git and an archiver. `faff`, `sha256sum` and `shasum` are deliberately **not** in that set: each is guarded by a presence test with a stated fallback, so its absence is a recorded reading rather than a failure. `cat` is the only one that is the steps' rather than the probe's; `chmod`, `mkdir` and `ln` are the self-test's fixture dependencies — without `chmod` there is no mode-0000 fixture for cases (c) and (e), and without `ln` the dangling symlink is simply absent, so case (d) would pass for the wrong reason. `rm` is the probe's own: `classify_path` deletes the scratch file after every byte read, and the self-test deletes its fixture root when it is done. `cksum`, `date` and `uname` are on the list not because they are unguarded but because each falls back to a degraded header reading, and knowing they were missing is worth the line. *Validate:* before the reading, run a presence check for every tool in that list inside the image as a workflow step and record the output in the provenance notes. Git and the archiver are for the checkout action, not for the probe: checkout uses git when present and falls back to downloading an API tarball when it is not, which then needs the archiver. It also injects its own Node runtime, so the image must be glibc-compatible — `debian:bookworm-slim` is, which is why it is the named candidate. If `cksum` is missing, the header falls back to `probe_bytes` and the note says so; if the shell is missing the image is not a candidate at all; if git and the archiver are both missing, checkout fails before the probe runs and the image must be replaced.
...
- [ ] No committed file under `records/spikes/2026-07-26-FAFF-654/` matches any of the three scan branches ported from `config.js:870–880`.
...
- [ ] `records/spikes/2026-07-26-FAFF-654/` exists and contains `probe.sh`, `RESULTS.md`, and per obtained shape: one probe transcript, one self-test output, and one pre-checkout listing file.
...
- [ ] The containerised job runs the tool-presence check before checkout, covering every tool the probe and the job depend on, and its output is pasted into `RESULTS.md`'s provenance notes.
