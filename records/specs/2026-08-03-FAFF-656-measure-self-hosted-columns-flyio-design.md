...
6. **Download + commit verbatim** into `records/spikes/2026-07-26-FAFF-654/`; fill the `selfhosted-direct` / `selfhosted-container` columns + `worktree_changed_by_checkout.selfhosted-*` in `RESULTS.md`, stamp `substrate: fly.io Machine (Firecracker microVM)` + image/size/region + the `count` mode choice in provenance.
