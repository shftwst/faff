...
**Problem statement.** `sumCodexUsage` reads three of the five fields codex reports on `turn.completed.usage`. Its `cache_write` class is declared, returned, and never written — a permanent zero for every codex backend, which reads as "this backend writes no cache" rather than "faff does not look". The two unread fields (`cache_write_input_tokens`, `reasoning_output_tokens`) were `0` in both live captures to date (codex-cli 0.145.0, `docs/reference/architecture/codex-cli-observed.md`), so the drop is latent rather than currently wrong. This change reads `cache_write_input_tokens` into `totals.cache_write` and acknowledges `reasoning_output_tokens` against the `output` class, or names in a comment why a field is deliberately not added.
...
| `docs/reference/architecture/codex-cli-observed.md` | Markdown | Records the two live captures (codex-cli 0.145.0); both new fields `0`. The committed real payloads the acceptance test reads. |
