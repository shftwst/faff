...
- **The frozen design records under `records/specs/*.md`.** Excluded: they are historical records of decisions made at a point in time and are deliberately exempted by the acceptance criterion — the name appears there as a factual account of the operator's setup at the time of writing. These are append-only history, not live fixtures.
...
PROCEDURE genericise_tailnet_host:
  ...
  2. Do NOT touch any file under records/specs/.
  3. Run the test suite; the deriveEgress / checkRealizable / eval-driver /
     ollama-model assertions MUST still pass unchanged.
  4. Run `git grep longhair-escalator`; expect hits ONLY under records/specs/*.md.
```

**Anti-pattern:** widening the replacement to `git grep -l` across the whole tree and rewriting `records/specs/*.md` too. Why: the acceptance criterion explicitly exempts the frozen design records; rewriting them corrupts historical accounts and fails review.
...
- The repo-wide check `git grep longhair-escalator` MUST return zero hits in any tracked path outside `records/specs/*.md`.
...
- **Assumes:** the seven occurrences listed in the reference table are the complete set of non-frozen-docs hits **among tracked files**. Validation: run `git grep -n "longhair-escalator" -- ':!records/specs/*.md'` before starting; expect exactly those seven lines. If more appear, widen the substitution to cover them (still excluding `records/specs/`). Note: `git grep` searches tracked files only — the acceptance criterion's "returns only records/specs paths" is scoped to tracked files (an untracked path such as `verification/external-verification/faff-labs/` is out of this ticket's declared scope, which was FAFF-618's for scaffolders).

## 7. DONE — Definition of Done

### From WHY
- [ ] No tracked file outside `records/specs/*.md` contains the literal `longhair-escalator` (verified by `git grep longhair-escalator` returning only `records/specs/*.md` paths).

### From WHAT
- [ ] Every replaced literal is exactly `http://studio.x.ts.net:11434` (scheme and `:11434` port preserved).
- [ ] No file under `records/specs/` is modified.
...
PROCEDURE smoke:
  ...
  4. Expect: hits only under records/specs/*.md.
