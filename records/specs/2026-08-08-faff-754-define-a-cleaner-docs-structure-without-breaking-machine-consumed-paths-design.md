...
- **`.faffrc.yaml` and the spec-path resolver:** `tracking.spec_docs_path` points to `records/specs/`.
- **`plugin/skills/faff/bin/lib/adr.js` and graft flows:** ADR commands and merge checks use `records/adr/`.
...
PROCEDURE trace_known_executable_surface:
  1. Select records/spikes/2026-07-26-FAFF-654/probe.sh.
