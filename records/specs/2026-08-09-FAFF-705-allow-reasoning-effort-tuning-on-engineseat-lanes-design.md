...
| `records/adr/0054-*.md` (FAFF-422) | Markdown | ADR whose "effort refused on engine lanes" consequence this ticket amends |
| `records/adr/0050-*.md` (FAFF-416) | Markdown | Establishes the three tunable effort lanes; engine lanes are two of them |

**Scope statement.** This sits at the seam between the per-lane effort surface (FAFF-416) and the engine-call transport layer (FAFF-422/FAFF-593/FAFF-481) — it is the tuning branch FAFF-593 explicitly deferred to a later ticket (`records/specs/2026-07-25-FAFF-593-*-design.md:39,225`).
