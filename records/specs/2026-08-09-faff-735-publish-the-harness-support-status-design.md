...
- **`docs/reference/architecture/harness-coupling.md`**: seam inventory and current portability dispositions, including a stale `codex --full-auto` reference.
- **`docs/reference/architecture/codex-cli-observed.md`**: dated Codex 0.145.0 observations for permissions, event streams, auth, skill loading, and state.
- **`records/adr/0090-engine-transport-gains-a-spawn-family-codex-extending-adr-0054-s-per-lane-transp.md`**: accepted read-only Codex engine transport boundary.
- **`verification/audits/2026-08-02-FAFF-435-l4-gate-subversion.md`**: committed result of the Codex/GPT-5.6-sol audit run.
...
- **Running the autonomous Codex B10 experiment**: FAFF-613 owns the scored P1-class run and committed result. Extension point: `verification/external-verification/**` under FAFF-613.
...
- `docs/reference/architecture/harness-coupling.md`, linking the current status page and correcting statements that exceed observed Codex behaviour, including the nonexistent `--full-auto` flag.
- `docs/reference/architecture/codex-cli-observed.md`, linking forward to the current support judgement while retaining the dated observation unchanged in substance.
...
`website/docusaurus.config.js` already reads `docs/guide` in place. The current guide sidebar is hand-authored in `website/sidebars-guide.js`, so the build adds the new page there without changing the docs plugin configuration.
...
- Replace `codex --full-auto` with the observed separate approval and sandbox axes from `docs/reference/architecture/codex-cli-observed.md`.
...
**Chosen:** make `docs/guide/harness-support.md` the canonical current support page and keep `docs/reference/architecture/harness-coupling.md` as the seam-design inventory.
...
**Does the website need custom navigation work?** The guide plugin reads `docs/guide` directly, while `website/sidebars-guide.js` now lists the published pages explicitly. The page therefore needs one sidebar entry to be discoverable.

**Chosen:** use the existing guide publication path and add the page to the current hand-authored sidebar.
...
**Validated input:** FAFF-732 is Done. Its committed public-claim audit was read before assigning support language.

**Validated input:** FAFF-733 is Done. Its canonical language is applied without changing package, command, repository, configuration, or URL identifiers.
...
- [ ] `docs/reference/architecture/harness-coupling.md` links the support page and no longer claims Codex has a `--full-auto` flag.
- [ ] `docs/reference/architecture/codex-cli-observed.md` links forward to current support while retaining its dated observed facts.
...
- **Codebase fit:** the guide plugin already publishes `docs/guide`, and the current guide sidebar is hand-authored. Resolution: add one entry to `website/sidebars-guide.js` without changing plugin configuration.
...
- **Dependency check:** FAFF-732 and FAFF-733 shipped before this build. Resolution: use their committed audit and language guide as the current inputs.
...
The repository's existing `faff lint-refs` gate also keeps `docs/guide/**` free of ticket identifiers and numbered ADR paths. The guide therefore links direct committed evidence, merged pull requests, and the harness coupling inventory. Ticket-level provenance remains in the architecture and specification surfaces where that gate permits it.
