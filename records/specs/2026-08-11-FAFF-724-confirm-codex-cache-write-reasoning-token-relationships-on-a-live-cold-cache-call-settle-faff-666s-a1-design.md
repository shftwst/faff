...
>
> Revised by human decision on 2026-08-19: the paid live non-zero capture requirement below is superseded. Official OpenAI Responses usage semantics plus the version-pinned Codex mapping are the acceptance evidence; the original text remains as the historical proposal. Reconfirmed on 2026-08-27 with the instruction to preserve the implementation and rerun review and CI. The verdict lives in the pre-existing `docs/reference/architecture/codex-cli-observed.md`; historical path references below omit the `reference/` segment.
