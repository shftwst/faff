...
**Threading the version through the read.** `read.version` must reach `classifyBundle`. The store's `headDigest` already parses `manifest.json`; have it also return `version` (both occupants, local and git-remote). `verifyBundleIdentity` then reads `head.version`, computes `required = requiredMembersFor(head.version)`, fetches member bytes for `required` (not the hardcoded 6), and passes `read.version = head.version` into `classifyBundle`.

**The integrity anchor for `version` is store immutability, not the digest (corrected).** `bundle_manifest_digest = sha256(canonicalJSON(memberRefs))` covers only the member-refs map, not `version` or `identity`, and it is a plain hash (not a keyed MAC). So an attacker with write access to a stored bundle can strip `contract_fingerprint`, set `manifest.version` to `"b1"`, and recompute the digest over the remaining six refs, and the verifier reads it CLEAN under the b1 required set. What actually protects `version` is the store occupant: the **git-remote** occupant is a write-once immutable ref (overwriting is out-of-band), which is the real anchor for the cross-box recovery path; the **local** occupant has no such anchor, so a local-store b2 to b1 downgrade-strip is not detected by the verifier and rests on filesystem trust. This is deferred, not live: `contract_fingerprint` has no reader yet. The follow-up FAFF-868 tracks the local-store version-integrity question, and the future posture-aware recovery gate that reads `contract_fingerprint` must not treat a `b1`-classified bundle as posture-verified without an authenticated version. (An earlier draft of this spec wrongly claimed `version` was digest-covered; it is not.)
