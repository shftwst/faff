...
| `plugin/skills/faffter-noon-review/SKILL.md` | Markdown prompt | Canonical source of the graft-time code review (five categories). The kit's `code-review/lens/review-lens.md` is the benchmark-shaped rendering of this review's categories. |
...
The code-review lens gets a **semantic** parity check rather than a byte-identical one, because its `code-review/lens/review-lens.md` is a legitimate rendering of `faffter-noon-review/SKILL.md`'s five review categories (AC coverage, obvious bugs, scope check, spec fidelity, human-judgement) with the `### <severity>:` output contract appended, not a verbatim slice. The parity test asserts those five category names are present and in order, so a canonical change that adds, drops, or reorders a category fails CI and names the divergence; a pure wording tweak inside a category does not, which is correct for a rendering. The build agent also adds a top-of-file comment in `review-lens.md` naming `faffter-noon-review/SKILL.md` as its canonical source, and the README documents the re-derive step.
...
- [ ] It asserts `eval/review-bench/code-review/lens/review-lens.md` contains the five canonical review categories from `faffter-noon-review/SKILL.md` (AC coverage, obvious bugs, scope check, spec fidelity, human-judgement) in order; that file carries a top-of-file comment naming its canonical source, and the README documents the re-derive step.
