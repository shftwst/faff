...
- [ ] It asserts `eval/review-bench/code-review/lens/review-lens.md` contains the five canonical review categories from `faffter-dark-adversarial-review/SKILL.md` (Specification gaming, Implicit assumptions, Failure mode blindness, Security surface, Concurrency and ordering) in order. The kit README names its canonical source (`faffter-dark-adversarial-review/SKILL.md`) and documents the re-derive step; the lens file is left pristine so the benchmark prompt matches what faff sends.
