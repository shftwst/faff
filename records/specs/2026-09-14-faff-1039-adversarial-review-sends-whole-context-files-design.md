...
> **Build note (2026-09-14, recorded across review rounds 4 and 5): the trim MECHANISM above was substituted, and this paragraph is the record of it, including the substitution's own limit.**
> Step 7's `trimContextFiles(..., thresholdBytes = trimTargetBytes(primaryWindow, diff))` does not reduce anything. `thresholdBytes` is an ON/OFF GATE in `trimContextFiles` (below it the trim is an identity no-op); it is never consulted again, and the aggressiveness lives in `window` / `headLines`. Re-running it at a different threshold returns byte-identical output.
> The build therefore keeps `trimTargetBytes` with its signature and purity intact and adds `tightenToTarget`, a descending ladder over the trim `window` that takes the MINIMUM across rungs (the ladder is not monotonic in bytes: a tighter window can stop a file being head-reduced and enlarge it). Every rung keeps all diff-touched lines, so it never degrades to the diff-only view. `trimTargetBytes` budgets everything in the prefix except the diff, and `tightenToTarget` compares against `prefixBytes - diffBytes`; comparing against the full prefix charges the diff twice.
>
> **What the substitution does NOT do, measured on this ticket's own review payload.** Round 4's adversarial call carried a real 12-file, 516,847-byte context. The default trim reduced it by **zero bytes**. Round 5 then ran the replacement over the same payload: every rung of the ladder came out at or above the untrimmed 666,225-byte prefix, `adopted` was false, and the note correctly printed "no rung shrank it ... KEEPING the untightened prefix". So on that payload this feature delivers fix-direction 4 (loud surfacing) and **none** of fix-direction 1. That is within scope, per the "even the tightened trim can't fit" edge case and the conditional wording of the first DONE item, but it must not be read as the substitution rescuing the measured case. It does not.
> The honest summary: the ladder reduces payloads whose context is reducible by windowing, and for the rest the value delivered is that the skip becomes loud and deterministic instead of a silent 400.
>
> One caveat on the reasoning above: "a payload large enough to overflow a context window is far above the 48KB default gate" compares against a gate that measures **context bytes only**. A small-context, large-diff payload can overflow a window without the default trim ever firing. The conclusion holds in the regime that matters here; the stated reason is broader than it holds.
> Every DONE item below is satisfied as written. The mechanism moved, and `PRIMARY_SKIP_SIGNAL` ships as `[faff:primary-skipped]` rather than the spec's `<<<FAFF-1039 primary-skip>>>`, mirroring the existing `TRUNCATION_SIGNAL` house shape.
