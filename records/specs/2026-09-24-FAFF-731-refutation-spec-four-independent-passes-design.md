...
- **The failure:** a sub-pass emits output that cannot be parsed *after normalisation*. The fan-out runs production's `normaliseCleanRefutation` → `parseRefutation` chain, so a brief-instructed clean form (`## Refutation — <lens>` + `No <lens> objection.`) folds to the canonical no-findings token and grades `clear`; only a genuinely-malformed response (neither a clean form nor a valid `### severity:` finding) fails to parse. **How you'd know:** `parseRefutation` returns `{ok:false}` after normalisation. **What it means:** error the rep — a lens that produced no gradeable output is a measurement failure the eval surfaces (mirroring how the collapsed eval treats a malformed envelope), never a silent drop or a spurious injected objection. A parse fault that turns out to be the common clean case is the missing-normalisation bug this chain exists to prevent.
