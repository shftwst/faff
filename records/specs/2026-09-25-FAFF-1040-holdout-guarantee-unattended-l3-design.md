...
> Revised 2026-09-25 (mid-build, operator-directed, second pass) — **the merge floor is simplified to the capability facts alone** (two postures: strict / pass-through), level-agnostic: a fresh, caged, code-blind verdict is enforced at any level; its absence steps aside, labelled; a faulted read fails safe to strict. The `offer` posture and the `unattended` mint-stamp/ledger-read were dropped: an offer surfaced at merge is useless (the operator has stepped away), so the active offer belongs at **build start** and is a **follow-up**, along with **capturing the run's capabilities in the run ledger** (so gates read captured capability facts instead of the L level or a merge-time artifact proxy). **ADR-0131 is the authoritative record of the shipped design;** where this spec body still describes three postures or a mint-stamped `unattended`, ADR-0131 supersedes it. The sections below are retained as the design narrative.
