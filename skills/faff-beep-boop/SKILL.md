...
```
node ~/.claude/skills/faff/faff.mjs runcheck   # audits the latest .faff/runs/* ledger
```
...
```json
{
  "hooks": {
    "Stop": [
      { "type": "command", "command": "node \"${CLAUDE_PLUGIN_ROOT}/skills/faff/faff.mjs\" runcheck --hook" }
    ]
  }
}
```
