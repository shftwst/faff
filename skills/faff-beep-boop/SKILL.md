...
- **No Bash approval prompts anywhere in the run.** Every `Bash` call made by beep-boop or any sub-skill it invokes must follow the gateway's **decompose, don't wrap** rule (see `skills/faff/SKILL.md` → **Bash command hygiene** — Rules 0, 0.5, 0.6 plus the banned-constructs table): atomic binary invocations only, no shell expansion or substitution, no wrapper scripts, no `cd <dir> && git ...` (use `git -C <dir>`), no `| head`/`| tail`/`| awk` (use `Read`/`Grep`). A single approval prompt anywhere in the pipeline — during workit's build/test/CI commands, during prep's tracker writes, during tidy's cleanup — halts the whole run. The cost of decomposition is always lower than the cost of a halt.
