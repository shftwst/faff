...
Issues that are **not automation-eligible** (gateway → **Automation eligibility**) **and** otherwise notable — i.e. an explicitly-held ticket (`faff-automation-hold`), or an otherwise-ready ticket that simply hasn't been blessed (`faff-automate`) under the opt-in default. (Do **not** list every unlabelled backlog ticket — under opt-in that's most of the backlog; scope this to held tickets + would-be-ready-but-unblessed ones.) **Distinct from "Stuck in prep":** parked work is automation-blocked (it tried and stopped); these are human-blocked pre-emptively, with no auto-clear. The job of this section is to keep held-back work visible **and** give the human a basis to decide *what to bless* — not just a list of ids.
...
**Bless-set proposals (rendered when a `methodology` slot answers `bless-set`).** After the flat On-hold list, request the `bless-set` output from the configured methodology (gateway → **The `methodology` slot**) and render the ranked batches as cards — the **root** via the On-hold-entry form + the **slice** via the value-chain arrow render (`rendering_adaptor`), with per-member badges (would-build / would-need-prep / would-still-need-decision from `faff next --if-eligible`), the hypothetical-verdict distribution one-liner, the stop-reason, and deferred-with-reason members. **Omit** when no methodology is configured / no not-eligible items / no qualifying root (degrades to the flat list above).

**Interactive batch-bless (per bless-set card).** In **interactive** tidy, end each Bless-set card with a single confirm: **"Bless this set (N items) for the next drain? (y/n)"** — listing every member explicitly first, defaulting to the **smallest coherent slice** (extending depth is an explicit opt-in). On `y`, apply the per-issue bless action — add `faff-automate` (ensure the label exists first, gateway → **Control-label provisioning**) to **exactly the listed members and no others**; on `n`, nothing changes. **Do not auto-promote** — members become automation-eligible and a subsequent `/faff-beep-boop` drains them prep→build (zero pipeline changes). Log the approved set + stop-reason + members to `.faff/runs/<run-id>/` for the calibration loop. **Human-gated at every appetite, including `full`** — autonomous tidy, `/faff-wtf`, and `/faff-beep-boop` never bless from this flow; appetite tunes which sets are *proposed* (the `bless-set` output), never this gate. This batches the existing single bless below; it changes nothing about beep-boop's eligibility-exclusion or the gateway invariant that *a not-eligible issue can never reach the build queue*.
