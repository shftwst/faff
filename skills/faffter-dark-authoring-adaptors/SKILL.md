...
| `ship_adaptor` | Delivery outcome — `shipped`/`not-ready`/`failed` + two-tier gate + coercion rule | the result envelope, native-result→outcome translation |
...
| `intake` / `spec` / `review` / `ship` (producers) | the paired adaptor maps their output; producers map onto **nothing** directly (`intake` is the exception — it emits a documented brief with no paired adaptor) | how the work is done; output shaped so the paired adaptor can translate it |
