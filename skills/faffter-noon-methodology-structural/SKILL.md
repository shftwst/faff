...
| **Repeat-park patterns** | Active issue parked 3+ times in last 21 days (configurable in `.faffrc`) with the same root-cause class | Reads last 50 `.faff/runs/*/summary.md` files; classifies each park by the root-cause class enum (the shared taxonomy owned by the `routing_contract` slot, default `faffidavit-routing`) |
...
Render a `### Structural diagnostics` section when any finding exists. Format follows the configured `language_contract` slot — cycles use the cycle bracket (≤3 edges) or cycle box (4+ edges) form. Example:
...
The cycle + ghost-project findings feed the **automation-routing contract** (the `routing_contract` slot, default `faffidavit-routing`): an issue in a detected cycle gets `circular-blocked`; an issue with a ghost-project pointer gets `gap-blocked`.
