...
| **Repeat-park patterns** | Active issue parked 3+ times in last 21 days with the same root-cause class | Reads last 50 `.faff/runs/*/summary.md` files; classifies each park by the root-cause class enum (the shared taxonomy fixed in the gateway's automation-routing contract; the `routing_adaptor` slot, default `faffidavit-routing`, assigns against it) |
