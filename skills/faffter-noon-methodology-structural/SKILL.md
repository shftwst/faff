...
| **Repeat-park patterns** | Active issue parked 3+ times in last 21 days (configurable in `.faffrc`) with the same root-cause class | Reads last 50 `.faff/runs/*/summary.md` files; classifies each park by the root-cause class enum (gateway-owned shared taxonomy) |
...
The cycle + ghost-project findings feed the gateway's **Automation-routing contract**: an issue in a detected cycle gets `circular-blocked`; an issue with a ghost-project pointer gets `gap-blocked`.
