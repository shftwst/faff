...
// FAFF-911: adversarial.max_tokens — the operator-configurable output-token cap for the
...
test("adversarial.max_tokens IS a writable scalar leaf", () => {
  const dir = tmpDir();
  writeFileSync(join(dir, ".faffrc.yaml"), "adversarial:\n  timeout: 120\n");
  const r = run(dir, "config", "set", "adversarial.max_tokens", "8000");
  assert.equal(r.code, 0, r.err);
  assert.equal(run(dir, "config", "get", "adversarial.max_tokens").out, "8000");
});

test("adversarial.spec_review.max_tokens IS a writable per-consumer scalar leaf", () => {
  const dir = tmpDir();
  writeFileSync(join(dir, ".faffrc.yaml"), "adversarial:\n  max_tokens: 8000\n");
  const r = run(dir, "config", "set", "adversarial.spec_review.max_tokens", "12000");
  assert.equal(r.code, 0, r.err);
  assert.equal(run(dir, "config", "get", "adversarial.spec_review.max_tokens").out, "12000");
  // the code_review consumer, unset, falls through to the global via the caller's -d read
  assert.equal(run(dir, "config", "get", "adversarial.code_review.max_tokens", "-d", "8000").out, "8000");
});

test("adversarial.max_tokens falls back to the -d default when unset", () => {
  const dir = tmpDir();
  writeFileSync(join(dir, ".faffrc.yaml"), "adversarial:\n  timeout: 120\n");
  // no max_tokens key present → the dispatch's `-d 2000` read yields the default
  assert.equal(run(dir, "config", "get", "adversarial.max_tokens", "-d", "2000").out, "2000");
});

// FAFF-911 negative test: the integer-normalisation guard both dispatch seams apply
// (`[ "$max_tokens" -gt 0 ] 2>/dev/null || max_tokens=2000`) resets a present-but-non-positive
...
function normaliseNumPredict(input) {
  const script = 'max_tokens="$1"; [ "$max_tokens" -gt 0 ] 2>/dev/null || max_tokens=2000; printf %s "$max_tokens"';
  return execFileSync("sh", ["-c", script, "sh", String(input)], { encoding: "utf8" });
}

test("max_tokens guard: a non-positive-integer value normalises to 2000, a positive integer passes", () => {
