...
test("profiles list: no override -> prints DELIVERY_PROFILE (today's exact vocabulary)", () => {
  ...
  try {
    ...
    assert.deepEqual(p.terminal_states, ["shipped", "pr-open", "parked", "errored", "routed-out", "unreached-budget", "superseded", "parked-window"]);
    assert.deepEqual(p.ledger_outcomes, ["shipped", "pr-open", "parked", "errored", "routed-out", "unreached-budget", "claimed-by-peer", "superseded", "parked-window"]);
