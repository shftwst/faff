...
test("registry keys are EXACTLY the 37 grader KINDS (total equality)", () => {
  const rk = Object.keys(registry.kinds);
  assert.equal(rk.length, 37);   // FAFF-996: +build-adjudication (FAFF-931: +spec-judge-discrimination)
...
test("seed is truthful: status:covered ⇔ ≥1 case in eval/cases/; designed are exactly the zero-case (live-lane) kinds", () => {
  ...
  assert.deepEqual(new Set(designed), new Set(["reconciliation", "verdict-build", "holdout-live", "prd-readiness", "park-reconsider-classification", "spec-judge-discrimination", "build-adjudication"]));
});
